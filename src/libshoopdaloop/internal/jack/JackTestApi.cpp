...
jack_client_t* JackTestApi::client_open(const char* name, jack_options_t options, jack_status_t* status, ...) {
    ...
    *status = (jack_status_t)0;
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Create client {} -> {}", name, (void*)rval);
    return rval;
}
...
void JackTestApi::init() {
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Initializing JackTestApi");
...
const char** JackTestApi::port_get_connections(const jack_port_t* port) {
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get all connections for port {} -> {} connections", (void*)port, conns.size());
    return rval;
}
    ...
    unsigned long bufsize)
{
    ...
    auto rval = p.as_ptr();
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Register port {} -> {}", port_name, (void*)rval);
    return rval;
};
    ...
    unsigned long  	flags)
{
    ...
    for (uint32_t i=0; i<ports.size(); i++) {
        ...
        logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get ports: {} -> {}", i, port_names[i]);
    }
    port_names[ports.size()] = nullptr;

    auto nports = ports.size();
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get ports: {} ports found", nports);
...
jack_port_t* JackTestApi::port_by_name(jack_client_t* client, const char *name) {
    ...
    logging::log<"Backend.JackTestApi", log_debug>(std::nullopt, std::nullopt, "name parts: {} : {}", cname, pname);
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get port by name {} -> {}", name, (void*)rval);
    return rval;
}
...
int JackTestApi::port_flags(const jack_port_t* port) {
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get port flags {} -> {}", (void*)port, rval);
    return rval;
}
...
const char* JackTestApi::port_type(const jack_port_t* port) {
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get port type {} -> {}", (void*)port, rval);
    return rval;
}

const char* JackTestApi::port_name(const jack_port_t* port) {
    auto rval = strdup(Port::from_ptr((jack_port_t*) port).name.c_str());
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get port name {} -> {}", (void*)port, rval);
    return rval;
}

const char* JackTestApi::get_client_name(jack_client_t *client) {
    auto rval = strdup(Client::from_ptr(client).name.c_str());
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Get client name {} -> {}", (void*)client, rval);
    return rval;
}

int JackTestApi::set_port_registration_callback(jack_client_t* client, JackPortRegistrationCallback cb, void* arg) {
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Set port registration cb for client {}, arg {}", (void*)client, arg);
...
int JackTestApi::connect(jack_client_t* client, const char* src, const char* dst) {
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Connect {} {}", _src.name, _dst.name);
...
int JackTestApi::disconnect(jack_client_t* client, const char* src, const char* dst) {
    ...
    logging::log<"Backend.JackTestApi", log_trace>(std::nullopt, std::nullopt, "Disconnect {} {}", _src.name, _dst.name);
