...
jack_client_t* JackTestApi::client_open(const char* name, jack_options_t options, jack_status_t* status, ...) {
    ...
    *status = (jack_status_t)0;
    ...
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Create client {} -> {}", name, (void*)rval);
    return rval;
}
...
void JackTestApi::init() {
    ...
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Initializing JackTestApi");
    ...
    unsigned long bufsize)
{
    ...
    auto rval = p.as_ptr();
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Register port {} -> {}", port_name, (void*)rval);
    return rval;
};
    ...
    unsigned long  	flags)
{
    ...
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Get ports: {} ports found", nports);
...
jack_port_t* JackTestApi::port_by_name(jack_client_t* client, const char *name) {
    ...
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Get port by name {} -> {}", name, (void*)rval);
    return rval;
}
...
const char* JackTestApi::port_type(const jack_port_t* port) {
    ...
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Get port type {} -> {}", (void*)port, rval);
    return rval;
}
...
int JackTestApi::set_port_registration_callback(jack_client_t* client, JackPortRegistrationCallback cb, void* arg) {
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Set port registration cb for client {}, arg {}", (void*)client, arg);
...
int JackTestApi::connect(jack_client_t* client, const char* src, const char* dst) {
    ...
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Connect {} {}", _src.name, _dst.name);
...
int JackTestApi::disconnect(jack_client_t* client, const char* src, const char* dst) {
    ...
    logging::log<"Backend.JackTestApi", log_level_debug_trace>(std::nullopt, std::nullopt, "Disconnect {} {}", _src.name, _dst.name);
