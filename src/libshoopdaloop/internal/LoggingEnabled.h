...
public:
    ...
    void log(std::format_string<First, Args...>&& fmt, First && first, Args &&... args) const
    {
        if constexpr (Level >= LevelFilter) {
            logging::log<Name, Level>(std::nullopt, std::nullopt, std::format(fmt, std::forward<First>(first), std::forward<Args>(args)...));
        }
    }
    ...
    void log(std::string const& str) const {
        if constexpr (Level >= LevelFilter) {
            logging::log<Name, Level>(std::nullopt, std::nullopt, str);
        }
    }
    ...
    {
        logging::log<Name, error>(std::nullopt, std::nullopt, std::format(fmt, std::forward<First>(first), std::forward<Args>(args)...));
        throw Exception("");
    }

    template<typename Exception>
    void throw_error(std::string const& str) const {
        logging::log<Name, error>(std::nullopt, std::nullopt, str);
        throw Exception("");
    }
