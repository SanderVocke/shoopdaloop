fn main() {
    ...
    cxx_build::bridge("src/cxx.rs")
        .std("c++20")
        .compile("tracing_cxx_bridge_cxx");

    let out_dir = std::env::var_os("OUT_DIR").unwrap();
    let out_dir = std::path::Path::new(&out_dir);

    // Export the include path for downstream crates.
    // Cargo will turn this into `DEP_TRACING_CXX_BRIDGE_INCLUDE`.
    println!(
        "cargo:include={}",
    ...
    // Cargo will turn this into `DEP_TRACING_CXX_BRIDGE_CXX_BRIDGE_LIBDIR`.
