...
impl Loop {
    ...
    pub fn add_audio_channel(
        ...
        let obj = *guard;
        let c_name =
            std::ffi::CString::new(name).map_err(|_| anyhow!("Failed to create CString"))?;
        let channel = unsafe {
            ffi::add_audio_channel(obj, mode as ffi::shoop_channel_mode_t, c_name.as_ptr())
        };
    ...
    pub fn add_midi_channel(
        ...
        let c_name =
            std::ffi::CString::new(name).map_err(|_| anyhow!("Failed to create CString"))?;
        let channel = unsafe {
            ffi::add_midi_channel(obj, mode as ffi::shoop_channel_mode_t, c_name.as_ptr())
        };
