...
For one selected regular or script composite loop, **details** instead shows its read-only timeline. Main tracks appear as horizontal rows, scheduled source loops appear as named rectangles, and overlapping loops on the same track stack into additional swimlanes. The timeline supports bounded horizontal zoom plus horizontal and vertical scrolling. Composition editing remains deferred; these controls change only the view.
