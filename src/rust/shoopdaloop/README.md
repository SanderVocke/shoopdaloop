...
The Shoop Lua API is versioned at major/minor ``1.5``. Every script must begin its Shoop API use with ``shoop_announce_api_version(1, 5)``; equal-major scripts with an equal or older minor run, while different majors, newer minors, missing calls, and malformed/repeated calls are cancelled before versioned side effects. The `shoop_control` module exposes the fixed Master as bus index `0`, with linear gain, fader-position, stereo balance, and mute getters/setters using the same authoritative application/backend path as the UI. Scripts may require ``shoop_dialog`` to create any number of named simple or paged windows containing portable rich text, Markdown with callback-backed links, file-backed Markdown, and optional-callback buttons. Files can be loaded only from paths below the Lua script's directory, either directly through ``shoop_file`` or with ``shoop_dialog.markdown_file``. When definitions are active, the top bar shows one count-bearing **Script Dialog(s)** menu; it is hidden when none exist. Scripts can request opening at startup or from callbacks; users control closing, reopening, and page selection until the owning script stops. See ``../../../docs/lua_dialog_api.md``.
