...
The browser renders the same Regular/Dry + Wet form and advertises **Built-in FX** and **Built-in Synth** from the AudioWorklet-backed processor catalog. They use the same fixed topology, embedded editors, and session state as native builds. Capture/destination ports are not presented as an External processor. Loading External, Carla, or another unavailable processor fails capability validation before worklet mutation and retains tracks, media, routes, and callback progress. Built-in FX rack state and Built-in Synth preset, sends, and MIDI assignments transfer unchanged between browser and native builds.
