...
The **Add Track** dialog offers **Regular**, channel-free **Trigger**, and **Dry + Wet**. Native capabilities always advertise External, **Tiny Synth/FX**, and **OxiSynth**; builds with `native-fx` also advertise Carla Rack, Patchbay, and Patchbay 16x. New Dry + Wet tracks use one shared count for matched dry and wet audio channels. External and Carla support optional dry MIDI, while the built-in synths require one MIDI input. OxiSynth always exposes two ignored dry audio inputs, two wet outputs, and one dry MIDI input. Its embedded editor selects the SoundFont and all 16 channel presets through direct Rust controls, reflects external MIDI overrides, and provides master gain, reverb, chorus, meters, favorites/recent presets, audition, and panic without creating recorded MIDI. Imported `.sf2` files are content-addressed and managed by the application; portable sessions embed only referenced user fonts. External tracks expose dry input/send and wet return/output ports in **Connections**; hosted processors keep FX endpoints internal and expose dry inputs, wet outputs, and dry MIDI. The all-tracks dialog also exposes **Global FX Control MIDI In**: CC 0–119, channel pressure, and pitch bend fan out to MIDI-capable FX without recording, automation, or waking sleeping DSP; inactive chains retain the latest values until normal processing resumes, with bounded restore over later blocks under saturation. Mappings remain processor-owned, and dual routing is additive. Loop playback can use recorded wet content or route recorded dry content through the processor, and wet recordings retain compatible restorable processor state.
...
The browser renders the same Regular/Dry + Wet form and advertises **Tiny Synth/FX** and **OxiSynth** from the AudioWorklet-backed processor catalog. It uses the same editor and reconstructible OxiSynth configuration as native builds. Imported SoundFonts are persisted in IndexedDB by digest and revalidated on reload. Portable sessions transfer referenced SoundFont payloads between browser and native builds; a missing digest opens recovery without replacing the running session. Capture/destination ports are not presented as an External processor. Loading External, Carla, or another unavailable processor fails capability validation before worklet mutation and retains tracks, media, routes, and callback progress.
