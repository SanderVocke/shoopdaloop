...
The browser requests echo cancellation, noise suppression, and automatic gain control off, but the browser may negotiate different settings. The engine runs at the context's actual sample rate and render quantum. The Connections dialog presents normalized ports as System sources → ShoopDaLoop sinks → ShoopDaLoop sources → System sinks, including negotiated `webaudio:capture_N`, `webaudio:destination_N`, and stable `webmidi:source|sink:<MIDIPort.id>` host ports. Ports remain grouped by client/device and track/script owner; Audio, MIDI, and track filters reduce the overview without changing routes. Drag from a source connector to a compatible sink to connect, or click a user-managed connection line to disconnect. Script-owned autoconnections are visible but not editable here. Audio and track-MIDI connection commands mutate authoritative worklet routes. Web MIDI input is assigned to frame zero of the next available quantum and output preserves engine order with browser scheduling latency; sample-exact timing is not claimed. All routed audio tracks sum with final clipping to `[-1, 1]`; input monitoring defaults off. See `../../../docs/port_model.md` and `../../../docs/web_midi_contract.md`.
