...
pub fn msgs_to_notes(msgs: impl Iterator<Item = MidiEvent>) -> Vec<Note> {
    ...
    fn terminate_note (start: i32, end: i32, channel: u8, note: u8, active_note_times: &mut Vec<Option<i32>>, notes: &mut Vec<Note>) {
        notes.push(Note {
            start_t: start,
            end_t: end,
            note: note,
    ...
    fn terminate_note_by_msg (event: &MidiEvent, active_note_times: &mut Vec<Option<i32>>, notes: &mut Vec<Note>) {
        terminate_note(
    ...
    fn terminate_channel_notes (channel: u8, time: i32, active_note_times: &mut Vec<Option<i32>>, notes: &mut Vec<Note>) {
        for note in 0..128 {
    ...
    for event in msgs {
        if is_note_on(&event.data) && !is_note_active(&event, &mut active_note_times) {
            ...
            terminate_channel_notes(channel(&event.data), event.time, &mut active_note_times, &mut notes);
