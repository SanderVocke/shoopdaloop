...
impl Backend {
    ...
    /// Driver state, for a caller that has the driver to hand.
    ///
    /// Takes the driver rather than holding one: a driver is owned by whoever started
    /// it, and a second owner is how two threads end up in the engine at once. The
    /// backend contributes the counters, the driver the rest.
    pub fn driver_state(&self, driver: &dyn crate::driver::Driver) -> crate::driver::DriverState {
        crate::driver::driver_state(driver)
    }
}
