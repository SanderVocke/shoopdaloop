...
#[test]
fn publishing_state_does_not_allocate() {
    use shoop_engine::engine::split;
    ...
    // Warm up and let the boxes reach their working size, as the control side does.
    for _ in 0..8 {
        engine.process(4);
        handle.poll();
    }
...
/// The same, with more loops than the snapshot boxes were sized for. The audio thread
/// must still not allocate; it publishes what fits and reports the shortfall.
#[test]
fn publishing_more_loops_than_fit_does_not_allocate() {
    use shoop_engine::engine::split;

    let mut s = Session::default();
    s.apply_graph_changes().unwrap();

    // Split first, so the snapshot boxes are sized for an empty session, and only then
    // add the loops. Creating them beforehand would size the boxes to fit and the
    // truncation path would never run.
    let (mut engine, mut handle) = split(s, 16);
    handle
        .send(Box::new(|s: &mut Session| {
            for _ in 0..64 {
                s.create_loop();
            }
    ...
    // Never polled, so nothing has grown the boxes: every cycle here has to publish a
    // truncated snapshot rather than reach for more room.
    assert_no_alloc(|| {
        for _ in 0..8 {
            engine.process(4);
        }
    ...
    // Confirm the truncation path really was the one exercised.
    let snap = handle.poll().expect("a published snapshot");
    assert_eq!(snap.n_loops, 64);
    assert!(snap.truncated(), "expected a truncated snapshot");
}
