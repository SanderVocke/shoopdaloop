...
//! without going through ALSA / CoreAudio / WASAPI. The real cpal host is platform
//! specific and on a fresh headless CI image there is no usable output device, so
//! the test was previously skipping (or worse, failing when the stub device exposed
//! by the loader could not be configured).
//!
//! `MockHost` implements the four cpal traits -- [`HostTrait`], [`DeviceTrait`],
//! [`StreamTrait`] -- the same way a platform host would: it reports a default
//! output and input device, and when asked for a stream it spawns a thread that
//! fires the data callback at the rate the configured sample rate and buffer size
//! imply. The thread keeps running until the stream is dropped, so the engine sees
//! the same callback cadence it would on real hardware.
//!
//! The mock implements [`DeviceTrait::build_output_stream_raw`] /
//! [`DeviceTrait::build_input_stream_raw`] directly rather than the typed
//! `build_output_stream` / `build_input_stream` defaults. The defaults go through
//! `&mut cpal::Data`, and `cpal::Data`'s only constructor is `pub(crate)` in cpal
//! 0.16, so a custom host cannot build one without `unsafe`. Because the typed
//! defaults call the raw variants, implementing the raw ones is sufficient: the
//! production code path (`build_output_stream(f32)`) lands in our raw override via
//! the trait's default wrapper.
