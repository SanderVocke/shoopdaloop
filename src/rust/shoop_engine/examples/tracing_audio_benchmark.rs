...
mod native {
    ...
    pub fn run() {
        ...
        let mut capture = mode.enabled().then(|| {
            ...
            .expect("start Perfetto benchmark capture")
        });
        if let Some(capture) = &capture {
            capture
                .wait_until_capturing()
                .expect("wait for Perfetto benchmark capture");
        }
        ...
        if let Some(capture) = &mut capture {
            capture
                ...
                .expect("discard Perfetto benchmark capture");
            shoop_common::tracing_capture::shutdown_reusable_profiler()
                .expect("shut down Perfetto benchmark profiler");
        }
    }
}
