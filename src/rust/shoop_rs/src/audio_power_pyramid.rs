...
pub fn create_audio_power_pyramid(input_data : &[f32], n_levels : usize) -> AudioPowerPyramidData {
    let mut current_level_power : Vec<f32> = input_data.iter().map(
        |v| {
            // Abs and clamp
            let v = f32::max(v.abs(), 10_f32.powf(-45_f32));
            // to dB
    ...
    let mut current_size = input_data.len();
    for i in 0..n_levels {
        if i>0 && current_size > 1 {
            ...
            for j in 0..current_size {
                current_level_power[j] = f32::max(prev_power[2*j], prev_power[2*j+1]);
            }
        }
        let level = AudioPowerPyramidLevelData {
            subsampling_factor : if current_size > 0 && input_data.len() > 0 { input_data.len() / current_size } else {1},
