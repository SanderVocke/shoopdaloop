...
impl ffi::ShoopEguiLoopWidget {
    ...
    fn set_loop_state(
        ...
        state.next_transition_delay = next_transition_delay;
        state.empty = empty;
        state.regular_composite = regular_composite;
        state.script_composite = script_composite;
        state.sync = sync;
        state.targeted = targeted;
        state.selected = selected;
        state.selected_composite_kind = selected_composite_kind;
    ...
    fn draw(&mut self, root_ui: &mut egui::Ui, _canvas: CanvasInfo) {
        ...
        egui::CentralPanel::default()
            ...
            .show(root_ui, |ui| {
                ...
                let has_transition =
                    state.next_transition_delay >= 0 && state.mode != state.next_mode;
                if has_transition {
                ...
                if icon_response.double_clicked() {
                    self.emit_icon_double_clicked();
                } else if icon_response.clicked() {
                    self.emit_icon_clicked();
                }
