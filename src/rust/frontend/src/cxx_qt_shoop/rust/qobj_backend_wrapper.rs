...
impl BackendWrapper {
    ...
    /// Synchronously pull state from the driver/session and push it to QML
    /// properties. This bypasses the normal async pipeline (UpdateThread timer
    /// → signal → queued slot) which can become unreliable when many test
    /// files are loaded sequentially in the same process.
    pub fn sync_update(mut self: Pin<&mut BackendWrapper>) {
        trace!("sync_update");

        {
            let ref_self = self.as_ref();
            if !ref_self.ready() {
                trace!("sync_update called on a BackendWrapper that is not ready");
                return;
            }
        }

        // Wait for the dummy driver thread to finish its current iteration
        // so the locks (driver, session) are free.
        {
        ...
        // Pull state from driver/session (same as update_on_other_thread).
        let update_data: BackendWrapperUpdateData;
        {
            let rust = self.as_mut().rust_mut();
            if rust.driver.is_none() || rust.session.is_none() {
                trace!("sync_update: driver or session missing");
                return;
            }
            let driver_state = match rust.driver.as_ref() {
                Some(d) => d.get_state(),
                None => return,
            };
            let session_state = match rust.session.as_ref() {
                Some(s) => s.get_state(),
                None => return,
            };
            let current_xruns = *self.xruns();
            update_data = BackendWrapperUpdateData {
                xruns: current_xruns + driver_state.xruns_since_last as i32,
                dsp_load: driver_state.dsp_load_percent,
                last_processed: driver_state.last_processed as i32,
                n_audio_buffers_available: session_state.n_audio_buffers_available as i32,
        ...
        // Push to QML properties (same as update_on_gui_thread).
        {
            self.as_mut().set_xruns(update_data.xruns);
            self.as_mut().set_dsp_load(update_data.dsp_load);
            self.as_mut().set_last_processed(update_data.last_processed);
        ...
        // Trigger cascaded updates to loop/port/fx backends.
        self.as_mut().updated_on_gui_thread();

        trace!("End sync_update");
    }
