...
pub fn invoke_connect_external_port<T>(
    obj: &mut T,
    connection_type: u32,
    port_name: QString,
    ...
    T: invokable::QObjectOrConvertible,
{
    ...
    invokable::invoke(
        obj,
        constants::INVOKABLE_BOOL_CONNECT_EXTERNAL_PORT.to_string(),
        connection_type,
        &args,
    )
}

pub fn invoke_determine_connections_state<T>(
    obj: &mut T,
    connection_type: u32,
) -> Result<cxx_qt_lib::QMap<cxx_qt_lib::QMapPair_QString_QVariant>, Exception>
where
    T: invokable::QObjectOrConvertible,
{
    invokable::invoke(
        obj,
        constants::INVOKABLE_DETERMINE_CONNECTIONS_STATE.to_string(),
        connection_type,
        &(),
    )
}
