...
{
    list.iter().map(|v : &QVariant| -> Result<T, anyhow::Error> {
        let converted : T = v.clone().try_into()
           .map_err(|_| anyhow::anyhow!("Error converting from QVariant to {:?}, metatype {:?}",
           type_name::<T>(),
                crate::cxx_qt_lib_shoop::qvariant_helpers::qvariant_type_name(v).unwrap_or("Unknown")
            ))?;
        Ok(converted)
    }).collect()
}
    ...
    QtT : QListElement + ExternType<Kind = cxx::kind::Trivial>,
{
    ...
    for elem in vec {
        let elem : QtT = elem.clone().try_into()
