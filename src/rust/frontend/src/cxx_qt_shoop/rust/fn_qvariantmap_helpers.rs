    ...
    T : TryFrom<QVariant> + Any,
{
    list.iter().map(|p : (&QString, &QVariant)| -> Result<(String, T), anyhow::Error> {
        ...
        let converted_val : T = p.1.clone().try_into()
            .map_err(|_| anyhow::anyhow!("Error converting from QVariant to {:?}, variant metatype is {:?}",
                type_name::<T>(),
                crate::cxx_qt_lib_shoop::qvariant_helpers::qvariant_type_name(p.1).unwrap_or("Unknown")
            ))?;
    ...
    T : QVariantValue + Any,
{
    list.iter().map(|p : (&QString, &QVariant)| -> Result<(String, T), anyhow::Error> {
        ...
        if !T::can_convert(p.1) {
            return Err(anyhow!("Cannot convert QVariant to {:?}, metatype is {:?}",
                type_name::<T>(),
                crate::cxx_qt_lib_shoop::qvariant_helpers::qvariant_type_name(p.1).unwrap_or("Unknown")
            ));
        }
