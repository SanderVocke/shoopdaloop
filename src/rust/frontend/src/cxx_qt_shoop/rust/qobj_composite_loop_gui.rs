use crate::{
    ...
    cxx_qt_shoop::qobj_composite_loop_gui_bridge::ffi::*,
    cxx_qt_shoop::qobj_loop_gui_bridge::ffi::qobject_to_loop_gui_ptr,
    loop_helpers::transition_frontend_loops,
...
use cxx_qt_lib_shoop::{
    ...
    qpointer::{qpointer_from_qobject, qpointer_to_qobject, QPointerQObject},
...
fn live_qobject(reference: &cxx::UniquePtr<QPointerQObject>) -> Option<*mut QObject> {
...
unsafe fn engine_identity(obj: *mut QObject) -> Option<LoopIdentity> {
    ...
    let basic = qobject_to_loop_gui_ptr(obj);
    if !basic.is_null() {
        return (&*basic)
            .rust()
            .backend_loop
    ...
    let composite = qobject_to_composite_loop_gui_ptr(obj);
...
impl CompositeLoopGui {
    ...
    fn install_engine_schedule_impl(mut self: Pin<&mut Self>) -> Result<(), anyhow::Error> {
        ...
        for (&start, events) in &self.schedule.data {
            for (target, explicit_mode) in &events.loops_start {
                let target_obj = live_qobject(&target.obj).ok_or_else(|| {
                    anyhow::anyhow!("composite schedule target is no longer available")
                })?;
                unsafe {
                    let dependency = qobject_to_composite_loop_gui_ptr(target_obj);
    ...
    pub fn list_transitions(
        ...
        for (&iteration, events) in self.schedule.data.range(start_cycle..=end_cycle) {
            ...
            iteration_transitions.extend(events.loops_end.iter().filter_map(|target| {
                live_qobject(&target.obj).map(|object| (object, LoopMode::Stopped))
            }));
            iteration_transitions.extend(events.loops_start.iter().filter_map(
                |(target, explicit_mode)| {
                    let object = live_qobject(&target.obj)?;
    ...
    fn collect_ringbuffer_adoptions(
        ...
        for (_, object) in nested {
            let dependency = unsafe { qobject_to_composite_loop_gui_ptr(object) };
    ...
    fn all_loops(self: &Self) -> Vec<*mut QObject> {
        ...
        for (_, events) in self.schedule.data.iter() {
            ...
            {
                if let Some(object) = live_qobject(&l.obj) {
    ...
    pub fn update_sync_position(mut self: Pin<&mut CompositeLoopGui>) {
    ...
    pub fn update_sync_length(mut self: Pin<&mut CompositeLoopGui>) {
    ...
    pub fn update_position(mut self: Pin<&mut CompositeLoopGui>) {
    ...
    pub fn update(mut self: Pin<&mut Self>) {
        ...
        if common::tracing_helpers::is_tracing_enabled() {
            let state_changed = self.mode != state.mode as i32
                || self.iteration != state.iteration as i32
                || self.position != state.position.min(i32::MAX as u64) as i32;
            if self.mode != state.mode as i32 {
                tracy_client::plot!("engine.composite.mode", state.mode as u32 as f64);
            }
            if self.iteration != state.iteration as i32 {
                tracy_client::plot!("engine.composite.iteration", state.iteration as f64);
            }
            if self.position != state.position.min(i32::MAX as u64) as i32 {
    ...
    pub fn clear(mut self: Pin<&mut CompositeLoopGui>) {
    ...
    pub fn metatype_name() -> String {
        unsafe {
            composite_loop_gui_metatype_name(std::ptr::null_mut())
...
pub fn register_qml_type(module_name: &str, type_name: &str) {
    ...
    unsafe {
        register_qml_type_composite_loop_gui(
            std::ptr::null_mut(),
            &mut module_name,
            1,
            0,
