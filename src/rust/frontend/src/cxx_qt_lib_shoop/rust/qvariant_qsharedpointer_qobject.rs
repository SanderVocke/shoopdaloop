...
mod ffi {
    unsafe extern "C++" {
        ...
        fn qvariantToQSharedPointerQObject(
            variant: &QVariant,
        ) -> Result<UniquePtr<QSharedPointer_QObject>>;

        #[rust_name = "qsharedpointer_qobject_to_qvariant"]
        unsafe fn qSharedPointerQObjectToQVariant(obj: &QSharedPointer_QObject)
            -> Result<QVariant>;
    }
}
...
pub fn qvariant_to_qsharedpointer_qobject(
    variant: &QVariant,
) -> Option<cxx::UniquePtr<ffi::QSharedPointer_QObject>> {
    ...
    match result {
        ...
        Err(err) => {
            println!(
                "Failed to convert QVariant to QSharedPointer<QObject>: {:?}",
                err
            );
            None
        }
...
pub fn qsharedpointer_qobject_to_qvariant(obj: &ffi::QSharedPointer_QObject) -> QVariant {
    unsafe { ffi::qsharedpointer_qobject_to_qvariant(obj).unwrap() }
}
