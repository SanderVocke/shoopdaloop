...
pub const SCRIPT_SETTINGS_KEY: &str = "script_settings";
pub const KEYBOARD_SCRIPT_FILENAME: &str = "keyboard.lua";
pub const AKAI_APC_MINI_MK1_SCRIPT_FILENAME: &str = "akai_apc_mini_mk1.lua";
...
pub struct KnownScript {
    pub path_or_filename: String,
    pub run: bool,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ScriptSettings {
    pub known_scripts: Vec<KnownScript>,
}
...
impl Default for ScriptSettings {
    fn default() -> Self {
        Self {
            known_scripts: vec![
                KnownScript {
                    path_or_filename: KEYBOARD_SCRIPT_FILENAME.to_owned(),
                    run: true,
                },
                KnownScript {
                    path_or_filename: AKAI_APC_MINI_MK1_SCRIPT_FILENAME.to_owned(),
                    run: false,
                },
            ],
        }
...
impl ScriptSettings {
    pub fn from_json(value: &Value) -> Result<Self> {
        UserSettings::from_json(value)?;
        let Some(settings) = value["configuration"].get(SCRIPT_SETTINGS_KEY) else {
            return Ok(Self::default());
        };
        if settings.get("schema").and_then(Value::as_str) != Some("script_settings.1") {
            return Err(anyhow!("unsupported or missing script settings schema"));
        }
        let known_scripts = settings
            .get("configuration")
            .and_then(|configuration| configuration.get("known_scripts"))
            .cloned()
            .ok_or_else(|| anyhow!("script settings are missing known_scripts"))?;
        let known_scripts: Vec<KnownScript> =
            serde_json::from_value(known_scripts).context("invalid known scripts in settings")?;
        if known_scripts
            .iter()
            .any(|script| script.path_or_filename.is_empty())
        {
            return Err(anyhow!("known script paths may not be empty"));
        }
        Ok(Self { known_scripts })
    }
    ...
    pub fn save(path: &Path, settings: &Self) -> Result<()> {
        let mut document = if path.exists() {
            read_settings_document(path)?
        } else {
            serde_json::json!({ "schema": "settings.1", "configuration": {} })
        };
        UserSettings::from_json(&document)?;
        let configuration = document["configuration"]
            .as_object_mut()
            .ok_or_else(|| anyhow!("settings are missing a configuration object"))?;
        let script_settings = configuration.entry(SCRIPT_SETTINGS_KEY).or_insert_with(|| {
            serde_json::json!({
                "schema": "script_settings.1",
                "configuration": {}
            })
        });
        if script_settings.get("schema").and_then(Value::as_str) != Some("script_settings.1") {
            return Err(anyhow!("unsupported or missing script settings schema"));
        }
        let script_configuration = script_settings
            .get_mut("configuration")
            .and_then(Value::as_object_mut)
            .ok_or_else(|| anyhow!("script settings are missing a configuration object"))?;
        script_configuration.insert(
            "known_scripts".to_owned(),
            serde_json::to_value(&settings.known_scripts)?,
        );
...
fn read_settings_document(path: &Path) -> Result<Value> {
    let bytes = std::fs::read(path)
        .with_context(|| format!("could not read settings from {}", path.display()))?;
...
fn write_settings_document(path: &Path, document: &Value) -> Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
    let parent = path.parent().unwrap_or_else(|| Path::new("."));
...
mod tests {
    ...
    #[test]
    fn script_settings_default_bundles_and_parse_legacy_entries() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("missing.json");
        assert_eq!(
            ScriptSettings::load(&path).unwrap(),
            ScriptSettings::default()
        );
        assert!(ScriptSettings::default().known_scripts[0].run);
        assert!(!ScriptSettings::default().known_scripts[1].run);
        let document = serde_json::json!({
            "schema": "settings.1",
            "configuration": {
                "script_settings": {
                    "schema": "script_settings.1",
                    "configuration": {"known_scripts": [
                        {"path_or_filename": "keyboard.lua", "run": false},
                        {"path_or_filename": "relative/controller.lua", "run": true}
                    ]}
                }
            }
        });
        assert_eq!(
            ScriptSettings::from_json(&document).unwrap().known_scripts[1],
            KnownScript {
                path_or_filename: "relative/controller.lua".to_owned(),
                run: true,
            }
        );
    }
    ...
    #[test]
    fn script_settings_save_is_atomic_and_preserves_other_service_fields() {
        let directory = tempfile::tempdir().unwrap();
        ...
        let original = serde_json::json!({
            "schema": "settings.1",
            "configuration": {
                "carla_hosting_mode": "subprocess",
                "midi_settings": {"schema": "midi_settings.1", "opaque": 7},
                "unknown_service": {"keep": true},
                "script_settings": {
                    "schema": "script_settings.1",
                    "configuration": {"known_scripts": [], "future": "keep"}
                }
            }
        });
        std::fs::write(&path, serde_json::to_vec(&original).unwrap()).unwrap();
        let settings = ScriptSettings {
            known_scripts: vec![KnownScript {
                path_or_filename: "controller.lua".to_owned(),
                run: true,
            }],
        };
        ScriptSettings::save(&path, &settings).unwrap();
        assert_eq!(ScriptSettings::load(&path).unwrap(), settings);
        let saved: Value = serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();
        assert_eq!(saved["configuration"]["midi_settings"]["opaque"], 7);
        assert_eq!(saved["configuration"]["unknown_service"]["keep"], true);
        assert_eq!(
            saved["configuration"]["script_settings"]["configuration"]["future"],
            "keep"
        );
        std::fs::write(&path, b"malformed").unwrap();
        assert!(ScriptSettings::save(&path, &settings).is_err());
        assert_eq!(std::fs::read(&path).unwrap(), b"malformed");
    }

    #[test]
    fn malformed_script_settings_are_observable_without_losing_defaults() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("settings.json");
        std::fs::write(
            &path,
            br#"{"schema":"settings.1","configuration":{"script_settings":{"schema":"script_settings.2","configuration":{}}}}"#,
        )
        .unwrap();
        let (settings, error) = ScriptSettings::load_or_default(&path);
