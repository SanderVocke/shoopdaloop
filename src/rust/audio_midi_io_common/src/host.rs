...
impl Host {
    ...
    /// is up to the caller.
    /// This function takes a callback which will be called once during the execution
    /// of new(). The caller will receive another callback which can be used to create
    /// a single processor on any thread except the thread this Host is used on.
    ...
    pub fn new(
        ...
        let mut create_processor = move || {
            let processor = HostProcessor::new(shared_state, receiver);
            Ok(processor)
        };
        create_processor_callback(Box::new(create_processor));
