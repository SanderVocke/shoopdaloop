...
void GraphLoopChannel::disconnect_port(std::shared_ptr<GraphPort> port, bool thread_safe) {
    (void)thread_safe;
...
WeakGraphNodeSet GraphLoopChannel::graph_node_0_incoming_edges() {
    ...
    if (auto in_locked = mp_input_port_mapping.lock()) {
        rval.insert(std::static_pointer_cast<GraphNode>(in_locked->first_graph_node()));
    }
    if (auto out_locked = mp_output_port_mapping.lock()) {
        rval.insert(std::static_pointer_cast<GraphNode>(out_locked->first_graph_node()));
    }
    return rval;
}
...
WeakGraphNodeSet GraphLoopChannel::graph_node_0_outgoing_edges() {
    WeakGraphNodeSet rval;
    if (auto l = loop.lock()) {
        rval.insert(std::static_pointer_cast<GraphNode>(l->graph_node()));
    }
    return rval;
}
...
WeakGraphNodeSet GraphLoopChannel::graph_node_1_incoming_edges() {
    ...
    if (auto in_locked = mp_input_port_mapping.lock()) {
        ...
        rval.insert(std::static_pointer_cast<GraphNode>(in_locked->second_graph_node()));
    } else {
...
WeakGraphNodeSet GraphLoopChannel::graph_node_1_outgoing_edges() {
    ...
    if (auto out_locked = mp_output_port_mapping.lock()) {
        ...
        rval.insert(std::static_pointer_cast<GraphNode>(out_locked->second_graph_node()));
    } else {
