...
void MidiStateTracker::copy_relevant_state(MidiStateTracker const &other) {
    fprintf(stderr, "[C++] DEBUG copy_relevant_state: tracking_notes=%d, tracking_controls=%d, tracking_programs=%d\n",
            tracking_notes(), tracking_controls(), tracking_programs());

    if (tracking_notes()) {
        auto n_notes = other.m_n_notes_active.load();
        fprintf(stderr, "[C++] DEBUG copy_relevant_state: copying %d notes_active\n", (int)n_notes);
        m_n_notes_active = other.m_n_notes_active.load();
        m_notes_active_velocities = other.m_notes_active_velocities;
    }
    if (tracking_controls()) {
        fprintf(stderr, "[C++] DEBUG copy_relevant_state: copying controls, pitch_wheel, channel_pressure\n");
        m_controls = other.m_controls;
        m_pitch_wheel = other.m_pitch_wheel;
        m_channel_pressure = other.m_channel_pressure;
    }
    if (tracking_programs()) {
        fprintf(stderr, "[C++] DEBUG copy_relevant_state: copying programs\n");
        m_programs = other.m_programs;
    }
}
...
std::vector<std::vector<uint8_t>> MidiStateTracker::state_as_messages() {
    ...
    fprintf(stderr, "[C++] DEBUG state_as_messages: tracking_notes=%d, tracking_controls=%d, tracking_programs=%d\n",
            tracking_notes(), tracking_controls(), tracking_programs());
    ...
    if (tracking_programs()) {
        for (uint8_t channel=0; channel < (uint8_t) m_programs.size(); channel++) {
            ...
            if (v != ProgramUnknown) {
                fprintf(stderr, "[C++] DEBUG: program ch=%d val=%d\n", channel, v);
    ...
    if (tracking_controls()) {
        for (uint8_t channel=0; channel < 16; channel++) {
            ...
            if (pw != PitchWheelUnknown && pw != PitchWheelDefault) {
                fprintf(stderr, "[C++] DEBUG: pitch wheel ch=%d val=%d\n", channel, pw);
                rval.push_back(pitchWheelChange(channel, pw));
            }
            if (cp != ChannelPressureUnknown) {
                fprintf(stderr, "[C++] DEBUG: channel pressure ch=%d val=%d\n", channel, cp);
            ...
            for (uint8_t controller=0; controller < 128; controller++) {
                ...
                if (v != default_cc(channel, controller)) {
                    fprintf(stderr, "[C++] DEBUG: CC ch=%d cc=%d val=%d\n", channel, controller, v);
    ...
    if (tracking_notes()) {
        for (uint8_t chan=0; chan < 16; chan++) {
            for (uint8_t note=0; note < 128; note++) {
                ...
                if (v != NoteInactive) {
                    fprintf(stderr, "[C++] DEBUG: note ch=%d note=%d vel=%d\n", chan, note, v);
    ...
    fprintf(stderr, "[C++] DEBUG state_as_messages: returning %zu messages\n", rval.size());
    return rval;    
}
