...
void DummyMidiPort::write_event(MidiStorageElem event) {
    ModuleLoggingEnabled<"Backend.DummyMidiPort">::log<log_level_debug>("Write midi message value to internal buffer @ {}", event.proc_time);
...
void DummyMidiPort::PROC_prepare(uint32_t nframes) {
    ...
    if (progress_by > 0 && m_queued_msgs.size() > 0) {
        ...
        std::erase_if(m_queued_msgs, [&, this](const MidiStorageElem &msg) {
            auto rval = msg.proc_time < progress_by;
            if (rval) {
        ...
        std::for_each(m_queued_msgs.begin(), m_queued_msgs.end(), [&](MidiStorageElem &msg) {
            auto new_val = msg.proc_time - progress_by;
            ModuleLoggingEnabled<"Backend.DummyMidiPort">::log<log_level_debug_trace>("msg in queue: time {} -> {}", msg.proc_time, new_val);
            msg.proc_time = new_val;
        });
    }
    ...
    if (m_direction == shoop_port_direction_t::ShoopPortDirection_Output) {
        ...
        if (!get_muted()) {
            for (auto &msg: m_buffer_data) {
                if (msg.proc_time < n_requested_frames) {
                    uint32_t new_time = msg.proc_time + (n_original_requested_frames - n_requested_frames);
                    ModuleLoggingEnabled<"Backend.DummyMidiPort">::log<log_level_debug>("Write midi message value to external queue @ {} -> {} {} {}", msg.proc_time, new_time, n_original_requested_frames.load(), n_requested_frames.load());
                    MidiStorageElem out_msg = msg;
                    out_msg.proc_time = new_time;
...
uint32_t DummyMidiPort::n_events() const {
    ...
    if (!m_queued_msgs.empty()) {
        for (auto it = m_queued_msgs.begin(); it != m_queued_msgs.end(); ++it) {
            if (it->proc_time < current_buf_frames) {
                r++;
            } else {
                break;
            }
