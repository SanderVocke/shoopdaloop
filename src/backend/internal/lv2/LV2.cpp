...
LV2::create_carla_chain(shoop_fx_chain_type_t type, uint32_t sample_rate, uint32_t buffer_size,
                        std::string title, shoop_shared_ptr<shoop_types::AudioBufferPool> maybe_buffer_pool) {
    if (!m_world) {
        if (!::g_initialized) {
        #ifdef _WIN32
...
LV2::create_carla_chain(
    shoop_fx_chain_type_t type, uint32_t sample_rate, uint32_t buffer_size, std::string title, shoop_shared_ptr<shoop_types::AudioBufferPool> maybe_buffer_pool);
