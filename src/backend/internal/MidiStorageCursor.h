...
public:
    using SharedCursor = std::shared_ptr<MidiStorageCursor>;
...
public:
    MidiStorageCursor(std::shared_ptr<IMidiStorage> storage);
    
    // For backward compatibility with specific storage types
    template<typename StorageType>
    MidiStorageCursor(std::shared_ptr<StorageType> storage);
    ...
    void set_storage(std::shared_ptr<IMidiStorage> storage);

private:
    std::shared_ptr<IMidiStorage> m_storage;
};