...
struct BackendSession::RecalculateGraphThread {
    ...
    RecalculateGraphThread(BackendSession &_backend)
        : finish(false), m_req_id(0), tmp_backend(&_backend) {
    ...
    void thread_fn() {
        ...
        while (true) {
            ...
            unsigned new_rid;
            cv.wait(lk, [this, &rid, &new_rid]() {
                new_rid = m_req_id.load();
                return finish || rid != new_rid;
            });
            if (finish) {
                break;
            }
            ...
            if (auto sh_backend = backend.lock()) {
                ...
                logging::log<"Backend.Session", log_level_debug>(
...
{
    audio_buffer_pool = std::static_pointer_cast<AudioBufferPool>(
        std::make_shared<BufferPool<shoop_types::audio_sample_t>>(
...
shoop_backend_session_state_info_t BackendSession::get_state() {
    ...
    rval.n_audio_buffers_created =
        audio_buffer_pool->n_buffers_created_since_last_checked();
    return rval;
}
        ...
        [this, &nframes]() {
            ...
            profiling::stopwatch(
                [this, &nframes, &processing_schedule]() {
                    ...
                    for (size_t i = 0; i < processing_schedule->steps.size();
                         i++) {
                        auto &step = processing_schedule->steps[i];
                        try {
                            if (step.nodes.size() == 1) {
                                log<log_level_debug_trace>(
                                    "[{}/{}] Processing node: {}", i, n_steps,
                                    step.nodes.begin()
                                        ->get()
                                        ->graph_node_name());
                                step.nodes.begin()->get()->PROC_process(
                                    nframes);
                            } else if (step.nodes.size() > 1) {
                                log<log_level_debug_trace>(
                                    "[{}/{}] Co-processing {} nodes, first: {}",
                                    i, n_steps, step.nodes.size(),
                                    step.nodes.begin()
                                        ->get()
                                        ->graph_node_name());
                                step.nodes.begin()->get()->PROC_co_process(
                                    step.nodes, nframes);
                            }
                        } catch (const std::exception &exp) {
                            log<log_level_error>(
                                "Exception while processing graph step: {}",
                                exp.what());
                        } catch (...) {
                            log<log_level_error>("Unknown exception while "
                                                 "processing graph step");
                        }
                    }
                },
...
std::shared_ptr<GraphLoop> BackendSession::create_loop() {
    ...
    auto loops_control_item =
        profiler->maybe_get_profiling_item("Process.Graph.Loops.Control");
    r->graph_node()->set_processed_cb(
        [loops_item, loops_control_item](uint32_t us) {
...
std::shared_ptr<GraphFXChain>
BackendSession::create_fx_chain(shoop_fx_chain_type_t type, const char *title) {
#ifdef SHOOP_HAVE_LV2
    static LV2 lv2;
#endif
    ...
    case Carla_Patchbay_16x:
        try {
            chain =
                std::static_pointer_cast<ProcessingChainInterface<Time, Size>>(
                    lv2.create_carla_chain<Time, Size>(
                        type, m_sample_rate, m_buffer_size, std::string(title),
                        audio_buffer_pool));
        } catch (const std::exception &exp) {
            throw_error<std::runtime_error>(
                "Failed to create a Carla LV2 instance: " +
                std::string(exp.what()));
        } catch (...) {
            throw_error<std::runtime_error>(
                "Failed to create a Carla LV2 instance (unknown exception)");
        }
        break;
#else
    case Carla_Rack:
...
#endif
    case Test2x2x1:
        chain = std::static_pointer_cast<ProcessingChainInterface<Time, Size>>(
            std::make_shared<CustomProcessingChain<Time, Size>>(
                2, 2, 1,
                [this, &chain](uint32_t n, auto &ins, auto &outs, auto &midis) {
                    ...
                    if (should_log<log_level_debug_trace>()) {
                        log<log_level_debug_trace>("FX output port 1: {}",
                                                   out_buf_1);
                        log<log_level_debug_trace>("FX output port 2: {}",
                                                   out_buf_2);
                    }

                    // Midi: for any MIDI message, synthesize a single sample on
                    // the timestamp of the message. Its value will be (3rd msg
                    // byte / 0xFF).
                    auto &midi = midis[0];
                    ...
                    memcpy((void *)outs[0]->PROC_get_buffer(n),
                           (void *)out_buf_1.data(), n * sizeof(float));
                    memcpy((void *)outs[1]->PROC_get_buffer(n),
                           (void *)out_buf_2.data(), n * sizeof(float));
                },
                audio_buffer_pool));
        break;
    };
    ...
    auto audio_ports_item =
        profiler->maybe_get_profiling_item("Process.Graph.Ports.Audio");
    auto midi_ports_item =
        profiler->maybe_get_profiling_item("Process.Graph.Ports.Midi");
    info->graph_node()->set_processed_cb(
        [fx_item](uint32_t us) { fx_item->log_time(us); });
    for (auto &p : info->audio_input_ports()) {
        p->first_graph_node()->set_processed_cb(
            [ports_item, audio_ports_item](uint32_t us) {
                ports_item->log_time(us);
                audio_ports_item->log_time(us);
            });
        p->second_graph_node()->set_processed_cb(
            [ports_item, audio_ports_item](uint32_t us) {
                ports_item->log_time(us);
                audio_ports_item->log_time(us);
            });
    ...
    for (auto &p : info->midi_input_ports()) {
        p->first_graph_node()->set_processed_cb(
            [ports_item, midi_ports_item](uint32_t us) {
                ports_item->log_time(us);
...
std::shared_ptr<GraphAudioPort>
BackendSession::add_audio_port(std::shared_ptr<shoop_types::_AudioPort> port) {
    auto rval = std::make_shared<GraphAudioPort>(port, shared_from_this());
    ports.push_back(std::static_pointer_cast<GraphPort>(rval));
...
std::shared_ptr<GraphMidiPort>
BackendSession::add_midi_port(std::shared_ptr<MidiPort> port) {
...
std::shared_ptr<GraphLoopChannel>
BackendSession::add_loop_channel(std::shared_ptr<GraphLoop> loop,
                                 std::shared_ptr<ChannelInterface> channel) {
    auto rval =
        std::make_shared<GraphLoopChannel>(channel, loop, shared_from_this());

    // Setup profiling
    auto loops_item = profiler->maybe_get_profiling_item("Process.Graph.Loops");
    auto loop_channels_item =
        profiler->maybe_get_profiling_item("Process.Graph.Loops.Channels");
...
BackendSession::~BackendSession() { destroy(); }

void BackendSession::set_sample_rate(uint32_t sr) { m_sample_rate = sr; }

void BackendSession::set_buffer_size(uint32_t bs) {
    auto notify = [&, this](auto &item) {
        auto as_node =
            dynamic_cast<NotifyProcessParametersInterface *>(item.get());
        if (as_node) {
...
void BackendSession::recalculate_processing_schedule(unsigned req_id) {
    ...
    for (auto &l : result->loops) {
        if (l) {
            weak_loop_nodes.insert(std::weak_ptr<GraphNode>(l->graph_node()));
        }
    }
    ...
    logging::log<"Backend.ProcessGraph", log_level_debug>(
        std::nullopt, std::nullopt, "Recalculating process graph");
    ...
    for (auto &l : result->loops) {
        if (l) {
            ...
            for (auto &c : l->mp_audio_channels) {
                if (c) {
                    insert_all(c);
                }
            }
            for (auto &c : l->mp_midi_channels) {
                if (c) {
                    insert_all(c);
                }
            }
    ...
    for (auto &c : result->fx_chains) {
        if (c) {
            insert_all(c);
            for (auto &p : c->audio_input_ports()) {
                if (p) {
                    insert_all(p);
                }
            }
            for (auto &p : c->audio_output_ports()) {
                if (p) {
                    insert_all(p);
                }
            }
            for (auto &p : c->midi_input_ports()) {
                if (p) {
                    insert_all(p);
                }
            }
    ...
    for (auto &n : nodes) {
        if (n) {
            raw_nodes.insert(n.get());
        }
    }

    // Schedule
    std::vector<std::set<GraphNode *>> schedule =
        graph_processing_order(raw_nodes);
    ...
    for (auto &s : schedule) {
        ProcessingStep step;
        for (auto &n : s) {
            step.nodes.insert(n->shared_from_this());
        }
    ...
    logging::log<"Backend.ProcessGraph", log_level_debug>(
        std::nullopt, std::nullopt, "Calculation took {} us", us);
    ...
    if (logging::should_log<"Backend.ProcessGraph", log_level_debug_trace>()) {
        ...
        logging::log<"Backend.ProcessGraph", log_level_debug_trace>(
            std::nullopt, std::nullopt, "DOT graph:\n{}", dot);
        if (logging::should_log("Backend.ProcessGraph", log_level_debug)) {
            ...
            logging::log<"Backend.ProcessGraph", log_level_debug>(
                std::nullopt, std::nullopt, "Processing schedule @ {}:\n{}",
    ...
    if (auto me = maybe_me.lock()) {
        auto finish_fn = [me, result, req_id]() {
            me->log<log_level_debug>("Applying updated process graph {}",
                                     req_id);
