...
uint32_t AudioMidiDriverRuntime::get_xruns() const { return m_rust_core->get_xruns(); }
void AudioMidiDriverRuntime::reset_xruns() { m_rust_core->reset_xruns(); }
void AudioMidiDriverRuntime::report_xrun() { m_rust_core->report_xrun(); }
float AudioMidiDriverRuntime::get_dsp_load() const { return m_rust_core->get_dsp_load(); }
void AudioMidiDriverRuntime::set_dsp_load(float load) { m_rust_core->set_dsp_load(load); }
uint32_t AudioMidiDriverRuntime::get_sample_rate() const { return m_rust_core->get_sample_rate(); }
void AudioMidiDriverRuntime::set_sample_rate(uint32_t sample_rate) { m_rust_core->set_sample_rate(sample_rate); }
uint32_t AudioMidiDriverRuntime::get_buffer_size() const { return m_rust_core->get_buffer_size(); }
void AudioMidiDriverRuntime::set_buffer_size(uint32_t buffer_size) { m_rust_core->set_buffer_size(buffer_size); }
const char* AudioMidiDriverRuntime::get_client_name() const { m_client_name_cache = std::string(m_rust_core->get_client_name()); return m_client_name_cache.c_str(); }
void AudioMidiDriverRuntime::set_client_name(const char* name) { m_rust_core->set_client_name(name ? name : "unknown"); }
void* AudioMidiDriverRuntime::get_maybe_client_handle() const { return reinterpret_cast<void*>(m_rust_core->get_client_handle()); }
void AudioMidiDriverRuntime::set_maybe_client_handle(void* handle) { m_rust_core->set_client_handle(reinterpret_cast<uintptr_t>(handle)); }
bool AudioMidiDriverRuntime::get_active() const { return m_rust_core->get_active(); }
void AudioMidiDriverRuntime::set_active(bool active) { m_rust_core->set_active(active); }
uint32_t AudioMidiDriverRuntime::get_last_processed() const { return m_rust_core->get_last_processed(); }
void AudioMidiDriverRuntime::set_last_processed(uint32_t nframes) { m_rust_core->set_last_processed(nframes); }
