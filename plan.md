...
- Formatting, diff checks, the closed 221-module inventory, and the warning-free workspace build pass. A release-with-debug no-run build of the tracing allocation test also passes, covering the release configuration used to archive CI tests. The warning-free frontend suite passes 32 tests; the serial all-feature engine suite passes 582 tests; and the complete workspace suite passes with ``SHOOP_ALLOW_MISSING_BACKENDS=1``, including 19 strict disabled-path allocation tests and the dedicated coarse/detail Tracy exception-scope test.
