...
| 12. Persistence and migration | Required session document version 10 fields and version-9-only neutral migration in `shoop_session`; archive validation rejects missing current fields, while cooperative/native/browser save/load, resampling, replacement, and driver-switch tests retain controls and exclude peaks. |
...
| 14. Delivery closure | Dedicated `shoopdaloop-mixer-step-2` branch and unified non-draft PR #843 against `master`; focused commits, exact-head required CI, nineteen root Codex findings with twenty evidence-backed replies, and final explicit Codex no-major-issues review are recorded on the PR. |
