...
| 9. Backend authority and bounded reconciliation | `ApplicationModel::desired_bus_controls`, authoritative snapshot overlay/settling, response-capable timeout aging, authoritatively confirmed and retrying compensating timeout commands, transactional submission, chained prior-accepted replay restoration on rejection, cancellable pre-commit session transfer, failed-transfer cleanup, atomic streamed complete-session replay retention, hotplug-safe cancellation, mutation failures, stale-identity cleanup, and large-session streaming without transfer-ACK buildup plus bounded command reservation with mandatory attach headroom; reconciliation, awaiting-gesture/route/host timeout and rejection, failed-submission/transfer cleanup, cancellation-boundary, saturation/headroom, and session-replay preflight tests. |
| 10. Target parity | `shoop_backend`, native adapter, fake backend, `shoop_audio_protocol`, `shoop_audio_worklet`, `shoop_worklet_client`, detached fixed-Master seed, explicit default-route disconnections, and raw host contract; native, complete Node Wasm, Chromium, Firefox packaged smoke, worklet DSP, protocol, replacement, early-control replay, and reconnect tests. |
...
| 12. Persistence and migration | Canonical exactly-one-Master empty/current documents, matching encode/decode bus-count validation, and required version-9/10 bus channels and mixer routes, version-9-only neutral control migration, and version-8-only empty-route migration in `shoop_session`; archive validation rejects missing current fields, while cooperative/native/browser save/load, resampling, replacement, and driver-switch tests retain controls and exclude peaks. |
| 13. Routing regression safety | Atomic internal graph schedules, explicit typed mixer routes, pair-scoped pending Connections facets, host-link replacement, and direct-route independence; engine/worklet fan-in/fan-out/disconnect tests, additive/fan-out dialog graph tests, session route tests, and no-allocation coverage. |
| 14. Delivery closure | Dedicated `shoopdaloop-mixer-step-2` branch and unified non-draft PR #843 against `master`; focused commits, exact-head required CI, twenty-eight root Codex findings with twenty-nine evidence-backed replies, and final explicit Codex no-major-issues review are recorded on the PR. |
