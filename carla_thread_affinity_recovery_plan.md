...
Validation evidence: test attribute policy, formatting, warning-denying workspace build, and tracing coverage all passed. The final complete CI-profile Rust run passed all 1,716 executed tests (four skipped). Real in-process and subprocess Carla UI/DSP probes passed on this system; the internal Audio Gain fixture covers the effect path, and opted-in in-process plus subprocess regressions loaded the system MDA ePiano, opened/closed Carla UI, injected MIDI, and verified audible-level generated samples without a crash or manual recovery. The final coarse trace had no loss/error stats and separated all UI callbacks from 7,240 DSP slices. Inspection confirms both deadline branches still zero wet/MIDI output and do not copy dry input.
