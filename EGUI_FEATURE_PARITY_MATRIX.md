...
| ARCH-002 | Presentation/business/backend separation | QML widgets currently own substantial session and control behavior. | Explored for M1 | Required | Complete | Separate `shoop_app_api`, `shoop_egui`, `shoop_app`, and `shoop_backend` crates with one-way dependencies; crate tests |
...
| ARCH-004 | Immutable snapshot and typed intent flow | The prototype has plain state/actions but receives snapshots and emits actions through QObject adapters. | Explored for M1 | Required | Complete | `shoop_app_api::tests::intents_preserve_stable_ids_and_selection_modifiers`, `shoop_app::tests::snapshot_reads_are_independent_of_actor_progress`, and bounded dispatch API |
...
| LAYOUT-004 | Sync track has a distinct fixed area and limited presentation | QML renders one non-editable sync track separately from main tracks. | Partially explored | Required subset | Partial — application snapshot distinguishes sync; presentation still treats tracks uniformly | `shoop_app::tests::actor_initializes_a_distinct_sync_track`; layout pending |
...
| BACKEND-002 | Poll loop, channel, port, and driver state | QObject update code currently converts state mirrors into QML properties and prototype snapshots. | Explored for M1 | Required | Partial — driver/loop polling is integrated; port/channel state follows direct topology | `shoop_backend` contract tests and `shoop_app::tests::actor_applies_intents_and_publishes_backend_state` |
| BACKEND-003 | Dummy-backend deterministic operation | Existing tests use a dummy backend for headless behavior. | Explored for M1 | Required | Complete | Shared backend contract passes for `FakeBackend` and `EngineBackend::new_dummy` |
