...
| Composite grab | Up to 64 primitive loop adoptions in one command | Frontend recursively flattens nested schedules into a stable request vector off RT. The callback validates every target and destination reserve, visits fixed rolling-buffer chunks directly, then commits all child data/mode changes in one command. The request vector returns with its command for non-RT destruction. |
...
| Composite grab | In scope; verified | At most 64 recursively flattened primitive adoptions; all targets/reserves validate before mutation. Fixed rolling buffers are visited directly and preallocated channel chunks are reused. Success and rejection are allocator-guarded. |
