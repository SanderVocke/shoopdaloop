...
This document describes the implemented Stage 1 compiled plan/state machine, Stage 2 engine sample-timeline integration, Stage 3 command/replacement/reclamation/snapshot slices, and the Stage 4 application/frontend adapter. The existing 24-case composite QML suite passes on the engine-backed path, and nested regular/script execution after session replacement is covered separately. Remaining callback-path lock/allocation work, running dependency-topology replacement, and legacy-adapter removal are called out explicitly rather than treated as complete. The implementation is in:
