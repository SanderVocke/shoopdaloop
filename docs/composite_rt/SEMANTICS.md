...
- **Running, unchanged dependency topology:** the candidate becomes the pending replacement. The newest accepted valid candidate supersedes an older pending replacement. The old plan remains authoritative through its current pass; replacement activates at the next iteration-zero boundary, where children absent from the new plan are stopped and new iteration-zero actions are resolved in one transaction. Same-target/same-mode children continue without a stop/start pair. A one-shot script has no natural next iteration zero, so its candidate activates in stopped state when the script completes and starts only in response to a later control/parent action.
- **Running or pending, changed dependency topology:** the complete candidate timeline activates at its callback command boundary. Active primitive children of the old topology are stopped first. Every retained running composite restarts in its previous mode at candidate iteration zero, nested actions settle in the candidate DAG, removed sources remain stopped, and new sources remain stopped unless reached by that restart propagation. Pending countdowns are canceled because their source/propagation context changed. This restart is one bounded boundary transaction and preserves the audio sample clock, diagnostics, and already accepted direct controls.
- If a running composite with an unchanged-topology candidate is stopped before its iteration-zero activation, cancellation settles first and the pending replacement activates in stopped state at that stop boundary.
