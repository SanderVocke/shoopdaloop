...
The native application's settings document (identified by `shoop-egui-settings` for compatibility) stores a global built-ins location, a dynamic ordered identity/toggle list, and an ordered user path/enabled list. Startup recursively discovers every valid `.lua` file from the packaged external tree. New identities default disabled; legacy keyboard/MK1 booleans migrate into the dynamic list when present. The one Settings dialog groups all script configuration and lifecycle controls by script kind in collapsible sections in its **Scripts** tab. The Example scripts section starts collapsed and makes examples such as `dialogs.lua` available to run on demand, with separate per-script Markdown documentation, log, and MIDI/status windows. Documentation is extracted verbatim from the Lua source's leading `--` comment block after removing the comment prefix. Persistent edits apply after Save; Stop, Restart, and Reload are runtime-only. Session scripts use independent `.shoop` resource bundles containing their Lua entrypoint and captured Markdown/PNG companions; they never persist machine paths.

Browser packages continue to use `wasm32-unknown-unknown` and exclude native MIDI dependencies. The cooperative browser application owner runs the same omniLua scripting manager as native: `scripting.supported` is true, a generated catalog fetches external scripts/resources from the packaged `builtins` root, and bundled session scripts use the shared transaction path. Browser settings register the catalog root and dynamic toggles; native user paths/Add-file remain absent. Before permission or on unsupported browsers, active logical registrations remain visible with no hosts. After explicit Web MIDI access, a bounded main-thread service supplies canonical physical endpoints, exact control input/output up to the existing 256-byte limit, hotplug reconnect, and owner-managed confirmed links independently of audio startup. Browser compatibility remains defined here; physical transport behavior is specified by `docs/web_midi_contract.md`.
