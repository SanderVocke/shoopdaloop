...
Loop channels remain ordered dry audio, wet audio, then optional dry MIDI and carry `mode: "dry"` or `mode: "wet"`. Carla tracks store the current opaque chain string in `fx_chain.internal_state`. A wet recording may reference an automatic `fx_states` entry through `recording_fx_state_id`; the entry's Carla chain type must match the current track. These strings are opaque UTF-8 and remain byte-for-byte significant, including newlines, Unicode, and NUL characters. Only referenced automatic take states are written.

Native External and advertised Carla processors are instantiated transactionally. A runtime whose processor catalog does not contain the required identity rejects the document before backend mutation. In particular, the current browser catalog is empty, so External and Carla sessions fail without replacing direct tracks or interrupting AudioWorklet progress.
