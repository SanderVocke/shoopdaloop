...
- Format-major 1 readers write session `document_version: 9`. Version 6 is accepted through an explicit migration that assigns zero capture alignment to every channel, versions 6 and 7 migrate the former processor advance to Manual processor-latency mode, and version 8 defaults the new mixer-route list and bus-channel metadata to empty. Older and newer document versions are rejected before session mutation.
...
The current application creates exactly one stereo Master bus on native and browser targets. New and legacy sessions start it disconnected unless document-version-9 state explicitly restores per-channel track routes or Master host links. Mixer routes reference stable track output port and bus channel IDs; Master output ports retain exact confirmed host IDs. The current application can instantiate direct and Built-in Synth sync/main track topology plus bundled session scripts on native and browser targets. Native builds additionally instantiate External and advertised Carla Rack/Patchbay/Patchbay16x dry/wet tracks, preserve role-bearing media and links across driver switches, and restore current and compatible recorded-take processor state before publication. Script bundle resources and source syntax are checked before commit, activated only after the shared session replacement commits, and captured exactly on save. Lua API compatibility is independent of this session format version; an incompatible script is retained without being runnable. Documents persist exact confirmed host IDs, including intentional disconnections, so session replacement removes startup defaults before restoring saved links. Additional buses, non-stereo Master shapes, bus processors, generic MIDI-control configuration, and session-local settings remain codec-representable but cause a capability error if runtime instantiation would be required. Unknown or unavailable track processors likewise fail transactionally rather than flattening to direct topology.
