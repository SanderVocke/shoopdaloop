...
Tiny Synth/FX `fx_chain` records may also contain a defaultable `midi_cc_assignments` list owned by ShoopDaLoop. Each entry identifies one of `master_gain`, `reverb_amount`, `distortion_drive`, `compressor_amount`, `eq_low`, `eq_mid`, `eq_high`, `vocoder_mix`, `vocoder_sensitivity`, or `noise_gate_threshold`, a zero-based MIDI channel in `0..=15`, and a controller in `0..=127`. Targets and channel/controller sources must each be unique. A missing list means no assignments. These mappings belong only to the current track chain and are not copied into automatic recorded-take `fx_states`.
