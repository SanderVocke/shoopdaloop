...
- a stateless OxiSynth topology and chain identity;
...
`OxiSynth` stores no variable topology fields. It always means exactly two dry audio inputs, exactly two wet audio outputs, and one dry MIDI input. The dry audio inputs provide the standard stereo track shape but their samples are ignored by the synth. Its chain type is also `OxiSynth` and its stable runtime processor ID is `oxisynth`. Document version 5 gives the chain a versioned JSON `internal_state` containing the SoundFont SHA-256 identity, master gain, reverb/chorus parameters, and exactly 16 baseline bank/program assignments. Runtime voices, external-MIDI overrides, audition state, meters, and effect tails are excluded. Version-4 empty states migrate to the embedded font and OxiSynth defaults.

The archive manifest may contain `soundfonts` records with a SHA-256 digest, original filename, declared uncompressed size, and the content-addressed path `soundfonts/<digest>.sf2`. Each referenced user payload is stored once and verified before parsing; the built-in font is identified by digest without embedding its bytes. Imported files are user-provided content: saving a portable session intentionally copies referenced bytes into the archive, and users remain responsible for any redistribution rights. A missing or mismatched payload never silently falls back to the built-in font; the decoded document remains a recovery candidate while the active session continues.
...
Native and browser runtimes instantiate Tiny Synth/FX and OxiSynth transactionally before publishing a replacement session. Native External and advertised Carla processors follow the existing native-only path. A runtime whose processor catalog does not contain the required identity rejects the document before backend mutation; browser builds therefore reject External and Carla without interrupting AudioWorklet progress, but preserve both built-in processors without flattening them.
