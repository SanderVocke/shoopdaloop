...
- Format-major 1 readers write session `document_version: 12`. Version 6 is accepted through an explicit migration that assigns zero capture alignment to every channel, versions 6 and 7 migrate the former processor advance to Manual processor-latency mode, versions 6–8 assign regular default playback to every track, pre-mixer documents receive disconnected mixer defaults, buses without controls receive `0 dB`/center/unmuted, and version 9 or 10 Built-in FX tracks migrate from fixed stereo/no-MIDI/state-v1 to matching channels plus a disconnected MIDI input and state-v2 defaults. Older and newer document versions are rejected before session mutation.
...
`OxiSynth` stores no variable channel fields. It always means exactly two dry audio inputs, exactly two wet audio outputs, and one dry MIDI input. The dry audio inputs preserve the standard stereo track shape but their samples are ignored by the synth. Its chain type and stable runtime processor ID remain `OxiSynth`/`oxisynth`; **Built-in Synth** is the display label. The chain's `internal_state` must contain valid version-2 OxiSynth state, and no automatic recorded-take `fx_state` is written. Session document version 12 is current; version 6 receives the explicit zero-alignment migration, versions 6 and 7 migrate their processor advance to Manual mode, versions 6–8 receive regular track default playback, pre-mixer documents receive disconnected mixer defaults, buses without controls receive neutral controls, and version 9 or 10 receives the Built-in FX migration described above.
