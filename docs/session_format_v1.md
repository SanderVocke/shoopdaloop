...
- an OxiSynth topology, chain identity, and selected-preset state;
...
`OxiSynth` stores no variable channel fields. It always means exactly two dry audio inputs, exactly two wet audio outputs, and one dry MIDI input. The dry audio inputs provide the standard stereo track shape but their samples are ignored by the synth. Its chain type is also `OxiSynth` and its stable runtime processor ID is `oxisynth`. The chain's `internal_state` must contain valid OxiSynth state, it may not contain Tiny Synth/FX MIDI mappings, and no automatic recorded-take `fx_state` is written. Document version 4 added the formerly stateless topology. Document version 5 requires explicit selected-preset state and migrates a valid version-4 empty state to the default preset. Versions 1 through 3 remain accepted and contain no OxiSynth topology.
