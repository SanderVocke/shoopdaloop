...
- Format-major 1 readers write session `document_version: 10`. Version 6 is accepted through an explicit migration that assigns zero capture alignment to every channel, versions 6 and 7 migrate the former processor advance to Manual processor-latency mode, versions 6–8 assign regular default playback to every track, and version 9 remains accepted as the immediate predecessor. Older and newer document versions are rejected before session mutation.
...
Built-in FX state is exactly `shoop-builtin-fx:1:<reverb-enabled>`, where the final field is canonical `0` or `1`. Effect tails and editor visibility are transient; only rack controls are persisted. Automatic recorded-take state may store the same control envelope.
