...
- Format-major 1 readers write session `document_version: 10`. Version 6 is accepted through an explicit migration that assigns zero capture alignment to every channel, versions 6 and 7 migrate the former processor advance to Manual processor-latency mode, versions 6–8 assign regular default playback to every track, and version 9 Built-in FX tracks migrate from fixed stereo/no-MIDI/state-v1 to stereo plus a disconnected MIDI input and state-v2 defaults. Older and newer document versions are rejected before session mutation.
