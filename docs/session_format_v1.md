...
`OxiSynth` stores no variable channel fields. It always means exactly two dry audio inputs, exactly two wet audio outputs, and one dry MIDI input. The dry audio inputs provide the standard stereo track shape but their samples are ignored by the synth. Its chain type is also `OxiSynth` and its stable runtime processor ID is `oxisynth`. The chain's `internal_state` must be empty, it may not contain Tiny Synth/FX MIDI mappings, and no automatic recorded-take `fx_state` is written. Document version 4 adds this topology; versions 1 through 3 remain accepted and migrate without changing their topology or processor state.
