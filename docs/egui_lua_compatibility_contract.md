...
Browser packages continue to use `wasm32-unknown-unknown` and exclude native MIDI. The cooperative browser application owner now runs the same omniLua scripting manager as native egui with an empty MIDI host service: `scripting.supported` is true, embedded keyboard/APC sources are present, keyboard is enabled by default, and source-bearing session scripts are syntax-checked, committed, and saved through the shared transaction path. Browser settings and end-to-end artifact evidence remain tracked by `plans/EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md`.
