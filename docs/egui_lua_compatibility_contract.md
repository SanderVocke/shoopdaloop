...
Browser packages continue to use `wasm32-unknown-unknown` and exclude native MIDI. The cooperative browser application owner runs the same omniLua scripting manager as native egui with an empty MIDI host service: `scripting.supported` is true, embedded keyboard/APC sources are present, keyboard defaults enabled/APC disabled, and source-bearing session scripts use the shared transaction path. Browser settings register and persist only bundled toggles; native user paths/Add-file remain absent. Active Lua MIDI registrations publish deterministic owner-managed application ports in global Connections, while the null service yields no MIDI hosts or confirmed links. End-to-end browser/artifact evidence remains tracked by `plans/EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md`.
