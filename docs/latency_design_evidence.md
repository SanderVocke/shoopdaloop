...
| Engine backend session transfer | `AudioContentReplaceItem`; `MidiContentReplaceItem`; `AudioDriverState`; Carla/OxiSynth control state strings | Add bounded take snapshots, retained bounds, alignment regions, and provider status. Validate before mutation. |
...
| Exact session document v6 | `SessionDocument`; `TrackDocument`; `LoopDocument`; `ChannelDocument` (`data_length_frames`, `start_offset_frames`, `preplay_frames`, recording FX reference); `PortDocument` (`ringbuffer_frames`); `FxChainDocument`; `FxStateDocument` | Add typed observation, policy, take alignment, retained bounds, warning, and optional bounded-region documents. Bump the exact accepted session document version. |
...
| Alignment regions retained per take | 256 | Bounded independently of media length and aligned with the existing per-operation channel bound. Consolidation is required before exceeding it. |
