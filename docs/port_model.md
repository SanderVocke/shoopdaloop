...
A host link is compatible when data types match and directions oppose. The application/backend boundary validates this before mutation. The Connections dialog derives a five-column graph from the normalized inventories and mixer state: System sources → ShoopDaLoop sinks → ShoopDaLoop sources → Buses → System sinks. Direct ShoopDaLoop-source-to-system-sink routes may bypass the Buses column. External system ports are grouped by client/device name and application ports by track or script owner. Host-inventory rows owned by the resolved ShoopDaLoop application instance are excluded from the outer system columns, so each ShoopDaLoop port appears only in its inner application column. Audio, MIDI, and multi-track filters hide graph content without changing connection state. Audio and MIDI endpoint labels, connectors, and routes use consistent type colors rather than letter prefixes; endpoint hover text names the type explicitly. Dragging a source connector to a compatible user-managed sink emits a typed `HostPortId`; confirmed routes are drawn as lines and a user-managed line can be clicked to disconnect. The graph does not retain a second candidate list.

Track and mixer links are user-managed. The fixed stereo **Master** bus has independent Left and Right summing inputs and host-connectable outputs. It starts with no track inputs and no system-sink links. A track Audio output may feed either or both Master channels while retaining any direct system links; multiple tracks feeding one channel are summed. Each bus row presents its input connector on the left and output connector on the right. Mixer links are backend-confirmed separately from host links and use the same pending, failure, timeout, and exact-disconnect truth model. Bus management, controls, processing, MIDI, and bus-to-bus routing are unavailable in this first version.
