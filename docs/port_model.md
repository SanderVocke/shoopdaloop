...
A link is compatible when data types match and directions oppose. The application/backend boundary validates this before mutation. The Connections dialog derives a four-column graph from the normalized inventories: System sources → ShoopDaLoop sinks → ShoopDaLoop sources → System sinks. System ports are grouped by client/device name and application ports by track or script owner. Audio, MIDI, and multi-track filters hide graph content without changing connection state. Dragging a source connector to a compatible user-managed sink emits a typed `HostPortId`; confirmed routes are drawn as lines and a user-managed line can be clicked to disconnect. The graph does not retain a second candidate list.
