...
public:
    ...
    void generate() {
        ...
        Expr is_starting = may_generate_sync_state(_states_in(loop)) && _positions_in(loop) == 0;
        Expr will_wrap = may_generate_sync_state(_states_in(loop)) && (_loop_lengths_in(loop) - _positions_in(loop)) < n_samples;
        // Debug prints
        // is_starting = print_when(is_starting, is_starting, loop, "starting, p:", _positions_in(loop), "l:", _loop_lengths_in(loop), "n:", n_samples);
