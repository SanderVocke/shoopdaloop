...
public:
    ...
    void PROC_process_channels(
        ...
        BasicLoop::PROC_process_channels(mode, maybe_next_mode, n_samples, pos_before, pos_after, length_before, length_after);

        for(auto &aloop : mp_audio_channels) {
            aloop->PROC_process(mode, maybe_next_mode, n_samples, pos_before, pos_after, length_before, length_after);
        }
        for(auto &mloop : mp_midi_channels) {
            mloop->PROC_process(mode, maybe_next_mode, n_samples, pos_before, pos_after, length_before, length_after);
        }
    }
