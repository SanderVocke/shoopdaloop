...
| G10 | Native release jobs in [run 31163582887](https://github.com/SanderVocke/shoopdaloop/actions/runs/31163582887) at implementation commit `ea9434d5`: Linux x64, Windows MSVC x64, macOS Intel, and macOS ARM ran the archived Rust matrix; `.github/actions/test_toplevel/action.yml` uploaded `carla-subprocess-benchmarks-*` CSV artifacts and each portable package ran the six-case Carla QML file with subprocess mode selected. The later repository-wide master policy disables non-Linux-debug jobs by default, but the test/action surfaces remain in-tree. | Native package/runtime/lifecycle/deadline, user-visible QML semantics, and direct/subprocess benchmark evidence on all supported OS families |
...
| REQ-09 | `SharedWorkerGuard`, bounded kill/reap, `SharedMemoryCleanup`, generation temp mappings | `requested_worker_shutdown_reaps_and_removes_shared_memory`; `worker_exits_and_cleans_ipc_after_abnormal_parent_termination`; shutdown soak; G6/G10 | Verified on Windows, Linux, macOS Intel, and macOS ARM |
...
| REQ-26 | portable std/TCP/UDP/tempfile/memmap implementation; same target-neutral lifecycle/UI model | G10 package, Rust, and QML results on Windows/Linux/macOS | Verified |
| REQ-27 | nonce/version/framing validation; random temp mapping; loopback launch/control; disconnect cleanup; bounded timeout/kill | protocol/shared tests plus the complete fake child-process matrix and abnormal-parent cleanup in G6, executed by G10 on all OS families | Verified |
| REQ-28 | self-spawn via `current_exe`; portable `shoopdaloop_exe`; quoted launchers | G8 passed from a non-ASCII path containing spaces; G10 package smoke passed on Windows, Linux, macOS Intel, and macOS ARM | Verified |
...
| REQ-33 | G9 Linux production report; real-Carla and deterministic fixture matrices in `shoopdaloop/tests/carla_worker.rs` emit p50/p95/worst/misses for 2/16 channels and 32–1,024 frames | G10 archived complete 24-row CSV files for Windows x64, Linux x64, macOS Intel, and macOS ARM | Verified |
...
- Future frontend contract: `plans/EGUI_FEATURE_PARITY_MATRIX.md`; browser/Wasm remains explicitly out of native hosting scope.

## Native evidence completion audit

The all-platform implementation evidence is [run 31163582887](https://github.com/SanderVocke/shoopdaloop/actions/runs/31163582887) at `ea9434d5`. Its Windows release/installer, Linux portable/AppImage, and macOS ARM release jobs completed successfully. The macOS Intel release job passed all 1,100 Rust tests—including every Carla worker, lifecycle, cleanup, deadline, and benchmark test—and its packaged six-case subprocess Carla QML run; unrelated QML persistence tests made that job's aggregate status fail afterward. All four native hosts produced complete 24-row real-Carla and fixture CSV artifacts covering both modes, both channel counts, and all six frame sizes. The run was canceled only after these release jobs and artifacts completed, when a newer master-merged branch head superseded its still-running Linux coverage job.

The current head differs from `ea9434d5` only through merged master CI/egui changes and this evidence documentation; no Carla implementation changed. Windows was also independently confirmed by successful targeted release/installer [run 31161049016](https://github.com/SanderVocke/shoopdaloop/actions/runs/31161049016). Wine and Linux-only results are not used as substitutes for any native acceptance row.
