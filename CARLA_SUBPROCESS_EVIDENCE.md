...
The only open evidence in this audit is native Windows/macOS execution and measurement. Linux-only compilation, Wine execution, or Linux timing is not used to satisfy those rows. The latest implementation-equivalent all-platform evidence run is [GitHub Actions run 31128435240](https://github.com/SanderVocke/shoopdaloop/actions/runs/31128435240) for commit `dbbd1377`; it is queued behind the active August 6, 2026 GitHub Actions service incident and is not evidence until it completes successfully.
