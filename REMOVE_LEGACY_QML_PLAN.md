...
- [x] Delete `.github/workflows/build_and_test.yml`, `.github/workflows/build_ci_containers.yml`, and the transitive legacy `.github/actions/` graph for vcpkg setup, qoverage, QML tests, screenshots, crash tests, old packages/installers, checkpoints, and package installation.
- [x] Remove obsolete manual CI-debug workflows that have no egui-specific purpose, and audit the remaining workflows for references to deleted paths. Preserve `build_and_test_egui.yml`, docs, CodeQL, and standalone Tracy tooling.
- [x] Tighten `build_and_test_egui.yml` into the sole application workflow: remove stale branch filters and old frontend isolation wording, add a warning-denying full retained-workspace test gate, keep formatting and tracing-inventory checks, and preserve all eight native/web build/package/test cells and browser smoke coverage.
...
- [x] Update `src/rust/shoopdaloop_egui/README.md`, `.agents/info/*.md`, `.agents/skills/tracy/`, and `.github/pi_coding_agent/build_instructions.md` so developer and agent commands use the retained egui tests/run path. Remove obsolete QML-only troubleshooting; retain generic CI contention guidance where still applicable.
