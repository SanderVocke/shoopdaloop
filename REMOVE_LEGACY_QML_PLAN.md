...
> Historical completed migration plan. Current crate, executable, and workflow names were adopted afterward.

## Goal and scope

Make `shoopdaloop` the only ShoopDaLoop application and remove the QML application's source, Rust integration layer, build/package infrastructure, CI, dependencies, and active documentation. The egui application is accepted as complete in its current form; this work does not add missing QML features or preserve QML feature parity.

The Cargo graph at the time showed that the retained native/WebAssembly closure did not use the legacy-only workspace packages `config`, `crashhandling`, `cxx_qt_lib_shoop`, `frontend`, `macros`, `midi_processing`, `packaging`, and `qt_header_bindings`, or the former QML application package. The legacy packaging path also owns `distribution/`, the repository-local `vcpkg/` overlays/bootstrap path, and the old `.github/actions/` graph. The retained egui packager currently reads `distribution/macos/icon.icns`; move that asset to an egui-owned resource before deleting `distribution/`.
...
- Renaming the retained Cargo package or executable.
...
1. The retained application is the only entry point. `src/qml/`, its `src/qml/third_party/QtMaterialDesignIcons` submodule gitlink, `.gitmodules`, the former QML application source, and all QML launch/self-test/package paths are absent.
...
- [x] Remove the `src/qml/third_party/QtMaterialDesignIcons` git submodule cleanly (gitlink and submodule metadata), delete the now-empty `.gitmodules`, then delete the rest of `src/qml/`, the former QML application crate, and the legacy-only crates `src/rust/{config,crashhandling,cxx_qt_lib_shoop,frontend,macros,midi_processing,packaging,qt_header_bindings}`.
...
- [x] Audit retained crates module-by-module for QML-only branches, helpers, test fixtures, comments, trace names, and public APIs. Remove dead paths; generalize surviving `shoop_common::tracing_capture`, engine/backend, scripting-key, and test-adapter terminology around the egui application.
...
- [x] Repoint `src/rust/shoopdaloop/package_artifacts.py` from `distribution/macos/icon.icns` to the identical retained icon under `resources/iconset/` (or another egui-owned resource location), then delete all of `distribution/`.
...
- [x] Delete the legacy application workflow, `.github/workflows/build_ci_containers.yml`, and the transitive legacy `.github/actions/` graph for vcpkg setup, qoverage, QML tests, screenshots, crash tests, old packages/installers, checkpoints, and package installation.
- [x] Remove obsolete manual CI-debug workflows that have no egui-specific purpose, and audit the remaining workflows for references to deleted paths. Preserve `build_and_test.yml`, docs, CodeQL, and standalone Tracy tooling.
- [x] Tighten `build_and_test.yml` into the sole application workflow: remove stale branch filters and old frontend isolation wording, add a warning-denying full retained-workspace test gate, keep formatting and tracing-inventory checks, and preserve all eight native/web build/package/test cells and browser smoke coverage.
- [x] Remove orphaned `codecov.yml` and the README badge unless egui Rust coverage is deliberately added. Replace stale Dependabot submodule/Python entries with automation for ecosystems that actually remain, or remove them.
- [x] Verify every local action reference and deleted workflow/path reference resolves cleanly; run the Linux debug and web debug workflow-equivalent commands locally, and use GitHub Actions validation for cross-OS YAML/expression behavior.

### Stage 4 — Rewrite README, installation, and maintained documentation

- [x] Rewrite `README.md` around the sole egui product: concise purpose/status, current native/browser capabilities and limitations, current screenshot allowance, build/docs links, actual CI badge, artifact types, and credits. Remove the QML roadmap, parity framing, stale backend/platform notes, unsupported MIDI-learn claims, and unaudited comparison-table claims.
- [x] Rewrite `INSTALL.md` from the egui workflow/package contract: Cargo/Trunk commands, per-platform native prerequisites, browser secure-context guidance, and unsigned native/web archive behavior. Remove submodule, Qt/QMake, vcpkg-prebuild, editable-QML, AppImage/Inno Setup/DMG, and old dev-launcher instructions.
- [x] Audit every page under `docs/` against current egui UI and tests. Rewrite architecture, track/loop/MIDI/Lua, tracing, ports, sessions/settings, browser, Carla, build/test, and packaging text; remove the generic QML MIDI-rule editor instructions and other unavailable UI while retaining implemented egui behavior.
- [x] Update `docs/tracing_coverage.csv` to exactly match retained production Rust modules and rewrite the tracing baseline/capture documentation around native egui rather than QML rotation/self-tests.
- [x] Update `src/rust/shoopdaloop/README.md`, `.agents/info/*.md`, `.agents/skills/tracy/`, and `.github/pi_coding_agent/build_instructions.md` so developer and agent commands use the retained egui tests/run path. Remove obsolete QML-only troubleshooting; retain generic CI contention guidance where still applicable.
- [x] Delete `plans/` and remove links to its parity/future/replacement documents. Historical screenshots may remain in `docs/source/resources/`, but references must not imply that their old controls are current.
- [x] Verify `sphinx-build -W --keep-going docs/source _build`, `python3 scripts/check_tracing_coverage.py --require-closed`, and a tracked-text link/path scan. Review all remaining `QML`, `Qt`, `cxx-qt`, `vcpkg`, removed-crate, old-workflow, and old-artifact matches against the explicit acceptance exceptions.
...
- [x] Run warning-denying native `shoopdaloop` default/no-default-feature builds and tests, artifact packaging/verification, and an offline/dummy application startup smoke test.
- [x] Run the WebAssembly checks for `shoopdaloop` and `shoop_audio_worklet`, Trunk debug/release builds, hosted/self-contained artifact verification, and dependency-isolation scans.
