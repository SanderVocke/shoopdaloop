...
from lib.q_objects.Backend import Backend, create_backend
...
be = create_backend('single_loop', app)
loop = be.create_loop()
audio1 = loop.add_audio_channel(True)
audio2 = loop.add_audio_channel(True)
midi = loop.add_midi_channel(True)
...
audio2_out = be.open_audio_port('audio_out_2', PortDirection.Output)
...
audio2.connect(audio2_in)
audio2.connect(audio2_out)
