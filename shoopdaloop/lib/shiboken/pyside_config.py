...
def get_shared_libraries_data(which_package):
    def glob_libs(dr, pattern=shared_library_glob_pattern()):
        glob_result = glob.glob(os.path.join(dr, pattern))
    ...
    package_libs = (glob_libs(package_path) if package_path else [])
    usr_libs = glob_libs(usrlib_path, 'libshiboken6.*.so*')

    return package_libs + usr_libs
