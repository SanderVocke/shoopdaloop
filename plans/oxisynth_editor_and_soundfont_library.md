...
- [x] Add OxiSynth editor descriptors, preset identities, configuration/snapshot types, typed controls, and track actions to the shared application API; advertise embedded UI and persistent state capability.
...
- [x] Add a reusable OxiSynth egui editor and connect it to track visibility using the capability-driven editor descriptor.
...
- [x] Add application library operations for import, list, inspect, remove-if-unreferenced, and explicit replacement with native durable storage, browser IndexedDB import/reload/deletion reconciliation, and management UI.
- [x] Make byte/catalog installation and removal atomic so restarts never advertise a digest whose payload is absent; native publication is staged behind payload/metadata commit and browser records become ready only after worklet commit acknowledgement.
