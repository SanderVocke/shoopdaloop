...
|---|---|---|---|---|---|---|
| LUA-ARCH-001 | Frontend-independent Lua ownership | QML wraps one `mlua` state per script; egui target is `shoop_scripting` on the application owner | Explored for Lua milestone | Required | Partial | Native actor constructs the non-`Send` script manager; shared environment/print installation is reused by the retained frontend adapter; target-gated dependency scan excludes browser/GUI. Application control integration remains open |
| LUA-RUN-001 | Lua 5.4 execution and bundled `require` | Sandboxed execution exposes Shoop print functions and only preloaded Shoop modules | Explored for Lua milestone | Required | Partial | Production sources are embedded and syntax-checked; isolated compatibility environment/require/print tests pass, while control bindings remain open |
...
| LUA-API-001 | Loop selector/query surface | Coordinates are `{track,row}`, sync is `{-1,0}`, and getters preserve list shapes/order | Explored for Lua milestone | Required | Complete | Framework-independent retained-case port covers coordinate/list parsing, ordered selectors, authoritative snapshots, all legacy names/constants, and read-your-writes behavior |
| LUA-API-002 | Loop mutation/transition surface | Trigger, explicit transition, grab/adopt, gain/balance, selection/target, clear, and repeat-sync | Explored for Lua milestone | Required | Complete | Ordered operations share application/backend policy; Fake/engine tests cover aligned/cycle transitions, adopt/grab, repeat-sync, gains, selection/target, clear, failures, and stale selectors |
| LUA-API-003 | Track query/mutation surface | Integer/list selectors include sync `-1`; linear gain, fader, balance, mute, and input controls | Explored for Lua milestone | Required | Complete | Full bindings, clamped conversion/shadow behavior, selector order, existing track reducers, backend effects, APC fader/mute workflow, and stale-ID tests pass |
| LUA-API-004 | Global control surface | Fixed cycles, solo, sync, play-after-record, and default record/grab are synchronous queries/mutations | Explored for Lua milestone | Required | Complete | Synchronous read-your-writes and shared reducers pass direct, keyboard, APC momentary/permanent, callback, and failure workflows |
| LUA-COMP-001 | Composition API required by APC | APC creates and extends regular compositions serially or in parallel | Explored for Lua milestone | Required | Complete | Coordinator-owned serial/parallel sections, derived length/state, clear teardown, primitive backend execution, production APC workflow, and EngineBackend execution test pass |
| LUA-EVENT-001 | Loop/global subscriptions | Committed changes produce typed non-reentrant callbacks with retained payloads | Explored for Lua milestone | Required | Complete | Committed-state diff, retained payload tables, queued post-callback operations, cloned callback dispatch, cross-script failure isolation, and application event tests pass |
| LUA-TIMER-001 | One-shot timers | Script-owned callback fires once after a non-negative delay | Explored for Lua milestone | Required | Complete | Monotonic elapsed-time countdown, deterministic one-shot removal, stop/drop ownership, callback operations, ordering, and error tests pass |
...
| LUA-UI-001 | Native script manager | Lifecycle, errors, docs, callbacks/timers, logs, MIDI diagnostics, file actions, and enablement are visible and controllable | Explored for Lua milestone | Required | Complete | Backend-free egui rendering/click tests, stable typed actions, composition-root picker/reload/remove/persistence adapters, archived logs, and explicit browser-unavailable presentation pass |
...
| LUA-MIDI-002 | Script-created logical MIDI outputs | Open/connected callbacks receive `send`; output broadcasts to matching external inputs | Explored for Lua milestone | Required | Complete | Existing port table API, opened/connected callbacks, bounded FIFO broadcast, reconnect, failure, and teardown tests pass |
...
| LUA-MIDI-004 | Bounded and rate-limited MIDI | Input/output queues are bounded; requested positive Hz is enforced and failures are observable | Explored for Lua milestone | Required | Complete | 1,024-message queues, 256-byte/callback bounds, monotonic FIFO spacing, `0` unthrottled behavior, drop/error/connection diagnostics, and exact timing/order tests pass |
| LUA-BUNDLE-001 | Production keyboard/APC workflows | Embedded shared sources control authoritative state with no egui script forks | Explored for Lua milestone | Required | Complete | Production keyboard command workflow and deterministic 8×8 APC initial reset/modifiers/globals/faders/LED/composition/hotplug/cleanup workflow pass; native virtual APC smoke explicitly skips when host MIDI is absent |
| LUA-SET-001 | Machine-wide script settings | `script_settings.1` lists bundled/user scripts; keyboard defaults enabled | Explored for Lua milestone | Required | Complete | Typed defaults/load/save, atomic preservation of MIDI/Carla/unknown fields, embedded bundle and user-file resolution, missing/malformed diagnostics, reload intents, and enablement persistence tests pass |
...
| LUA-PRES-001 | Native egui script manager | Add, enable, stop, restart, forget, help, status, errors, listening, and MIDI diagnostics | Explored for Lua milestone | Required | Partial | Plain immutable script state and typed stable-ID intents exist; egui manager rendering/routing remains open |
...
| LUA-BUILTIN-002 | Unchanged APC Mini MK1 script | Grid/modifiers, globals, faders, LEDs, timer, composition, MIDI I/O, and reconnect all work | Explored for Lua milestone | Required | Complete | Deterministic 8×8 fake-controller workflow plus platform-gated native virtual smoke; shared N-cycle coordinate bug corrected without an egui fork |
| LUA-WASM-001 | Browser preservation and explicit limitation | Current `wasm32-unknown-unknown` product has neither supported `mlua` target nor Web MIDI | Explored for Lua milestone | Required preservation | Partial | `shoop_app` compiles for `wasm32-unknown-unknown` with unsupported scripting state and no scripting/Lua/native-MIDI dependency; full browser regression gates remain final-stage work |
