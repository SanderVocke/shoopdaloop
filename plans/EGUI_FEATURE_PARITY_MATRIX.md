...
| FX-SUBPROC-003 | Bounded realtime block transfer | Direct Carla ran inline under a callback-visible mutex. | Partially explored | Partial | The session now owns a lock-free single-writer endpoint with preallocated audio/MIDI pools, three generation-tagged slots, bounded deadline fallback, local wake and authenticated worker notification; full session allocation/lock guards and Linux percentile measurements pass |
| FX-SUBPROC-004 | Checkpoint and click recovery | Session state lives in the live direct host. | Partially explored | Partial | Parent checkpoint retention, classified supervised restart, and QML toggle-or-recover semantics exist; direct and subprocess QML dry/wet activation/MIDI suites pass, while pure-egui FX remains deferred |
