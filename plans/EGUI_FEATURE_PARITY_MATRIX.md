...
- Engine path: physical `ExternalAudioPort` staging, deterministic mono/stereo mapping and mixing, actual-quantum processing, 120-second hard-bounded channel storage, visible low/exhausted counters, and tests proving exhaustion stops safely without render allocation.
...
Tiny implementation evidence current on 2026-08-10 includes hosted/self-contained Chrome Web Audio and Web MIDI, hosted Firefox Web Audio, output-only/offline/lifecycle/saturation/stress workflows, debug/release web and native packages, warning-denying native/Wasm/worklet builds, realtime guards, and default/native-FX tests. The integrated implementation commit `9e05415f` passed all Linux/Windows/macOS/WebAssembly debug/release cells in egui CI run `31436349108`; Linux workspace/QML run `31436349093` passed 1,294 Rust tests, 236 retained QML cases, and six packaged Carla subprocess cases. Local Qt execution remains unavailable, but the authoritative Qt-enabled CI gate supplies that evidence. Physical hardware checks remain an explicit environment limitation rather than claimed deterministic evidence.
