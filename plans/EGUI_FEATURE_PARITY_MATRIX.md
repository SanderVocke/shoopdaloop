...
`EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md` is the immutable implementation contract for the rows below. Investigation confirmed that stock vendored `mlua 0.11` fails for `wasm32-unknown-unknown`, while a minimal omniLua 0.7.1 Lua 5.4 probe compiles for that target. The user selected omniLua as the sole project runtime; complete Shoop and retained frontend/QML compatibility remains a blocking Stage-0 gate rather than assumed evidence. These rows intentionally replace Web Audio's special fixed-route/unsupported-connection behavior and the prior browser scripting omission, and remove `mlua` workspace-wide, without claiming Web MIDI or native egui real-driver composition.

| ID | Capability or behavior | Current baseline | Discovery | Milestone target | Current implementation | Planned evidence |
|---|---|---|---|---|---|---|
| XPORT-ARCH-001 | Explicit application-port and host-port inventories | Backend candidates are repeated per local port and ownership is track-only | Explored for cross-target ports/Lua | Required | Planned | Normalized API/backend contracts, owner identity, structural sharing, fake/dummy/Web Audio tests |
| XPORT-OWNER-001 | Track and Lua-control app-port ownership | Global egui connections include only sync/main tracks | Explored for cross-target ports/Lua | Required | Planned | Track scope isolation; global track/control aggregation; script lifecycle churn tests |
| XPORT-EMPTY-001 | App ports remain visible with no host endpoints | A category with no candidates displays only a no-endpoints message | Explored for cross-target ports/Lua | Required | Planned | Header/local-name paint tests for MIDI tracks and Lua control ports with empty Web MIDI inventory |
| XPORT-WEB-001 | Browser destination and microphone host endpoints | Web Audio maps every track directly and reports connection management unavailable | Explored for cross-target ports/Lua | Required | Planned | Lifecycle-driven endpoint snapshots and hosted/self-contained connection-dialog automation |
| XPORT-WEB-002 | Mutable authoritative worklet audio routes | Worklet protocol has no route command or confirmed connection state | Explored for cross-target ports/Lua | Required | Planned | Bounded protocol, actual connected/disconnected signal flow, pending/confirmation/failure evidence |
| XPORT-WEB-003 | Visible initial mapping and route persistence | Mono/stereo mapping is hidden fixed DSP behavior and is not persisted as connections | Explored for cross-target ports/Lua | Required | Planned | Ordinary disconnectable initial links, explicit-none round trip, and deterministic legacy-session migration |
| XPORT-MIDI-001 | Browser MIDI track ports with empty host inventory | Browser creates local MIDI descriptors but shows no usable matrix | Explored for cross-target ports/Lua | Required | Planned | Direct MIDI track creation and no-candidate global/track dialog evidence without invented endpoints |
