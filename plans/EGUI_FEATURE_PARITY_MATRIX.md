...
The detailed entries cover the completed tracks/loops, cross-target engine, browser-audio, track-port connections, session-persistence/loop-I/O, settings, and native Lua milestones, plus the planned cross-target ports/browser-Lua/omniLua-migration follow-up. Areas outside those slices remain listed coarsely until their milestone discovery begins.
...
- Historical M4 delivery published typed browser local descriptors but reported physical connection management unavailable. The cross-target ports follow-up now supersedes that limitation in source with normalized host inventories and mutable authoritative worklet routes; real browser automation remains before that follow-up is complete. Native real-driver selection remains separate.
...
| CONN-WASM-001 | Browser-compatible presentation | QML path is native; pure egui presentation must compile for Wasm. | Explored for connections | Connections required | Complete | Historical GUI/preview Wasm checks passed; follow-up source now renders normalized mutable Web Audio routes and awaits browser automation |
...
| CONN-DEF-004 | MIDI-control and other non-track ports | Global QML session dialog aggregates track ports, not the control port. | Re-explored for cross-target ports/Lua | Superseded target | Planned | `EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md` adds explicit non-track ownership and Lua-created logical control ports to global scope |
...
| FX-SUBPROC-006 | Pure-egui FX integration | FX chains and dry/wet tracks are deferred. | Partially explored | Deferred | Shared core is frontend-independent; the generic egui settings foundation is complete, while FX intents/snapshots and Carla-specific registration/presentation remain deferred |
...
| LUA-SESSION-001 | Source-bearing session scripts | `.shoop` stages script source before commit and preserves machine scripts | Explored for Lua milestone | Required | Complete | Exact ID/name/source/enabled round trip, pre-commit syntax rejection, post-commit activation/replacement, sample-rate cancellation rollback, machine/session separation, and browser rejection checks pass |
| LUA-PRES-001 | Native egui Scripts settings tab | Add, enable, stop, restart, forget, help, status, errors, listening, and MIDI diagnostics | Explored for Lua milestone | Required | Complete | One Settings entry point/window; typed drafts and runtime-only actions; lifecycle/help/log/activity plus aggregate/per-rule MIDI endpoint/failure diagnostics; browser omits the tab |
...
| LUA-WASM-001 | Historical browser preservation and explicit limitation | Current `wasm32-unknown-unknown` product has neither supported vendored `mlua` nor Web MIDI | Re-explored for cross-target ports/Lua | Superseded target | Complete current limitation; replacement planned | Existing isolation evidence remains valid; `EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md` mandates compatibility-gated omniLua while retaining an empty Web MIDI host inventory |
...
## Planned cross-target ports, browser Lua, and omniLua migration follow-up

`EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md` is the immutable implementation contract for the rows below. Stock vendored `mlua 0.11` failed for `wasm32-unknown-unknown`; the selected omniLua 0.7.1 runtime has since passed shared native and retained frontend/QML compatibility suites and compiles in the production browser composition. Normalized ports and authoritative worklet routes are implemented and unit-tested, while real browser/artifact evidence and Lua control-port/settings integration remain open. These rows do not claim Web MIDI or native egui real-driver composition.

| ID | Capability or behavior | Current baseline | Discovery | Milestone target | Current implementation | Planned evidence |
|---|---|---|---|---|---|---|
...
| XPORT-OWNER-001 | Track and Lua-control app-port ownership | Owner enum distinguishes sync/main tracks and Lua registrations; track publication is migrated | Explored for cross-target ports/Lua | Required | In progress | Global Lua-control aggregation and lifecycle churn remain Stage 5 |
| XPORT-EMPTY-001 | App ports remain visible with no host endpoints | Normalized UI and Web Audio backend preserve app columns/ports with an empty host inventory | Explored for cross-target ports/Lua | Required | In progress | Track/no-host GUI and backend tests pass; Lua control-port browser evidence remains |
| XPORT-WEB-001 | Browser destination and microphone host endpoints | Production Wasm configures negotiated capture/destination channels as stable `webaudio:*` host descriptors | Explored for cross-target ports/Lua | Required | In progress | Native worklet snapshots and Wasm compile pass; hosted/self-contained automation remains |
| XPORT-WEB-002 | Mutable authoritative worklet audio routes | Protocol v3 commands mutate render routing; only worklet snapshots confirm links; failures are nonfatal | Explored for cross-target ports/Lua | Required | In progress | Bounded protocol and actual connected/disconnected allocation-free signal-flow tests pass; browser UI run remains |
| XPORT-WEB-003 | Visible initial mapping and route persistence | Mono/stereo defaults are explicit confirmed links; exact-none session replacement and marker-based legacy migration pass | Explored for cross-target ports/Lua | Required | In progress | Unit/session evidence passes; hosted save/load and package workflows remain |
| XPORT-MIDI-001 | Browser MIDI track ports with empty host inventory | MIDI app descriptors remain normalized when browser host MIDI inventory is empty | Explored for cross-target ports/Lua | Required | In progress | Target-neutral no-host dialog tests and production Wasm compile pass; real browser dialog evidence remains |
| XPORT-CTRL-001 | Lua-created logical MIDI ports in global connections | Script ports exist only inside MIDI diagnostics/service state | Explored for cross-target ports/Lua | Required | Planned | Stable script/rule ownership, policy-managed native link truth, stop/restart cleanup, zero-endpoint Wasm state |
| XLUA-RUNTIME-001 | One omniLua Lua 5.4 compatibility implementation | Shared scripting and retained frontend now use pinned omniLua 0.7.1; native scripting passes and the crate compiles for Wasm without native MIDI | Explored for cross-target ports/Lua | Blocking prerequisite | In progress | All 61 functions/constants, callbacks/timers/errors, and embedded sources pass natively; production-browser execution remains open |
| XLUA-FRONTEND-001 | Retained frontend/QML on omniLua | Frontend engine, conversions, callbacks, ownership, session control, and MIDI bridges use omniLua directly | Explored for omniLua migration | Required | Complete | 33 frontend Rust tests and all Lua-specific retained QML self-tests pass; full offscreen suite is 235/236 with only an environment-unavailable CPAL case |
| XLUA-REMOVE-001 | Workspace-wide former-runtime removal | Manifests, Rust sources, metadata, lockfile, and resolved trees now contain only omniLua and its pure-Rust chain | Explored for omniLua migration | Required | In progress | Package scans plus final workspace/QML reruns remain before Complete |
| XLUA-WASM-001 | Browser application-owner scripting | Cooperative app scripting, reducers/events/compositions, key dispatch, source-bearing sessions, and empty MIDI service now compile in production Wasm; `scripting.supported` is true | Explored for cross-target ports/Lua | Required | In progress | Real hosted/self-contained browser runtime workflows remain required |
| XLUA-BUNDLE-001 | Embedded keyboard/APC in hosted and standalone HTML | Both sources are compiled into the production Wasm composition; keyboard starts enabled and APC disabled | Explored for cross-target ports/Lua | Required | In progress | Package inspection plus real keyboard authoritative and zero-endpoint APC browser workflows |
| XLUA-SET-001 | Browser bundled-script settings | Browser omits the Scripts registry/category | Explored for cross-target ports/Lua | Required | Planned | `localStorage` save/reload, keyboard-on/APC-off defaults, no dead native path picker |
| XLUA-SESSION-001 | Browser source-bearing session scripts | Wasm now compiles shared pre-commit validation, post-commit activation, rollback, capture, and exact save path | Explored for cross-target ports/Lua | Required | In progress | Real browser transaction and exact-byte round trip remain required |
| XLUA-KEY-001 | Browser keyboard control independent of audio permission | Wasm now compiles egui press/release translation through the enabled embedded keyboard script into authoritative reducers | Explored for cross-target ports/Lua | Required | In progress | Real browser focus-loss/text-entry and authoritative keyboard workflow remain required |
| XPORT-E2E-001 | Cross-target artifacts and regression closure | Web artifacts contain worklet routing but no Lua runtime | Explored for cross-target ports/Lua | Required | Planned | Chrome/Firefox/direct-file workflows, realtime gates, workspace/QML suites, dependency/package audits |
...
Driver/device selection, generic MIDI-rule editing, runnable Carla/FX settings, and session-local overrides remain assigned to their owning milestones. Native Lua startup settings now use this service; browser composition omits those definitions and preserves them as unknown values.
