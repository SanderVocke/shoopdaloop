...
| XLUA-WASM-001 | Browser application-owner scripting | App reducers, events, key dispatch, and snapshots are `cfg`-disabled | Explored for cross-target ports/Lua | Required | Planned | Cooperative omniLua runtime workflows and `scripting.supported == true` in production artifacts |
| XLUA-BUNDLE-001 | Embedded keyboard/APC in hosted and standalone HTML | Sources are embedded in a native-only scripting dependency | Explored for cross-target ports/Lua | Required | Planned | omniLua package inspection plus keyboard authoritative workflow and healthy zero-endpoint APC workflow |
...
| XLUA-SESSION-001 | Browser source-bearing session scripts | Browser capability validation rejects scripts and omits them on save | Explored for cross-target ports/Lua | Required | Planned | Pre-commit syntax validation, post-commit activation, rollback, and exact browser round trip |
| XLUA-KEY-001 | Browser keyboard control independent of audio permission | egui emits typed keys but browser app discards them | Explored for cross-target ports/Lua | Required | Planned | Press/release, focus loss, text-entry suppression, and unchanged keyboard-script browser workflow |
