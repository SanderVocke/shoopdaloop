...
| LUA-API-001 | Loop selector/query surface | Coordinates are `{track,row}`, sync is `{-1,0}`, and getters preserve list shapes/order | Explored for Lua milestone | Required | Partial | Function presence and representative selector/read-your-writes cases pass; the 45-case retained QML table and invalid-argument/return-shape matrix are not yet fully ported |
| LUA-API-002 | Loop mutation/transition surface | Trigger, explicit transition, grab/adopt, gain/balance, selection/target, clear, and repeat-sync | Explored for Lua milestone | Required | Partial | Ordered operations and representative Fake/engine paths pass; exhaustive retained cases, GUI equivalence, stale failures, and regular serial composition timing remain open |
| LUA-API-003 | Track query/mutation surface | Integer/list selectors include sync `-1`; linear gain, fader, balance, mute, and input controls | Explored for Lua milestone | Required | Partial | Bindings/conversions and APC fader/mute paths pass; complete selector/order/clamp/error parity table remains open |
| LUA-API-004 | Global control surface | Fixed cycles, solo, sync, play-after-record, and default record/grab are synchronous queries/mutations | Explored for Lua milestone | Required | Partial | Synchronous shadow updates and bundled workflows pass; complete retained validation/equivalence cases remain open |
| LUA-COMP-001 | Composition API required by APC | APC creates and extends regular compositions serially or in parallel | Explored for Lua milestone | Required | Partial | Sections, derived state, persistence, and parallel primitive starts exist; serial timing when synchronization is inactive is not yet authoritative |
| LUA-EVENT-001 | Loop/global subscriptions | Committed changes produce typed non-reentrant callbacks with retained payloads | Explored for Lua milestone | Required | Partial | Representative dispatch and cross-script failure isolation pass; all fields/kinds, registration-during-dispatch, duplicate suppression, and app ordering need explicit tests |
| LUA-TIMER-001 | One-shot timers | Script-owned callback fires once after a non-negative delay | Explored for Lua milestone | Required | Partial | Basic monotonic one-shot and teardown pass; multi-timer order, callback cap, cancellation, and re-registration edge cases need direct tests |
...
| LUA-UI-001 | Native script manager | Lifecycle, errors, docs, callbacks/timers, logs, MIDI diagnostics, file actions, and enablement are visible and controllable | Explored for Lua milestone | Required | Partial | Core rendering/actions/logs and browser-unavailable presentation pass; per-rule endpoint/failure diagnostics and invalid-startup path/ID association remain open |
...
| LUA-MIDI-002 | Script-created logical MIDI outputs | Open/connected callbacks receive `send`; output broadcasts to matching external inputs | Explored for Lua milestone | Required | Partial | Port API, bounded FIFO, reconnect, failure, and teardown pass; strict non-bursting positive-rate behavior and explicit multi-output broadcast timing need stronger tests |
...
| LUA-MIDI-004 | Bounded and rate-limited MIDI | Input/output queues are bounded; requested positive Hz is enforced and failures are observable | Explored for Lua milestone | Required | Partial | Queue/message/callback bounds and aggregate counters pass; accumulated elapsed time can currently flush a same-pump burst and app/UI diagnostics lack per-rule matched endpoint state |
| LUA-BUNDLE-001 | Production keyboard/APC workflows | Embedded shared sources control authoritative state with no egui script forks | Explored for Lua milestone | Required | Partial | Broad production keyboard/APC workflows pass; authoritative serial composition timing and strict output-rate evidence remain open |
| LUA-SET-001 | Machine-wide script settings | `script_settings.1` lists bundled/user scripts; keyboard defaults enabled | Explored for Lua milestone | Required | Partial | Typed preservation/default/file tests pass; startup state IDs are currently zipped with configured paths and can be mis-associated after an earlier invalid script |
...
| LUA-PRES-001 | Native egui script manager | Add, enable, stop, restart, forget, help, status, errors, listening, and MIDI diagnostics | Explored for Lua milestone | Required | Partial | Lifecycle/actions/help/log/aggregate diagnostics and browser-unavailable state pass; granular MIDI rules/endpoints/failures and startup mapping regression remain open |
...
| LUA-BUILTIN-002 | Unchanged APC Mini MK1 script | Grid/modifiers, globals, faders, LEDs, timer, composition, MIDI I/O, and reconnect all work | Explored for Lua milestone | Required | Partial | Deterministic 8×8 workflow and native virtual smoke exist; serial composition timing and strict non-bursting LED output-rate evidence remain open; shared N-cycle bug fix is documented |
| LUA-WASM-001 | Browser preservation and explicit limitation | Current `wasm32-unknown-unknown` product has neither supported `mlua` target nor Web MIDI | Explored for Lua milestone | Required preservation | Complete | Runner/UI/worklet compile for `wasm32-unknown-unknown`; browser explicitly reports unsupported scripting and rejects script sessions; dependency trees exclude scripting/Lua/native MIDI |
