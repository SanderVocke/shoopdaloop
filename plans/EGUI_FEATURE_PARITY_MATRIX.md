...
| TRACK-007 | Dry/wet Add Track choices | QML supports external and Carla processing with dry/wet audio/MIDI topology. | Explored for dry/wet/Carla | Dry/wet/Carla required | In progress | Typed/native stages and the capability-driven Regular/Dry + Wet egui form are complete, including independent counts, MIDI, constraints, browser-empty explanation, synthetic future providers, and typed acceptance; role-aware persistence remains |
...
The native QML Carla path supplies a complete frontend-independent hosting-mode setting, processor seam, supervised worker lifecycle, bounded generation logs, shared-memory audio/MIDI transport, checkpoint recovery, and status/log adapter baseline. Native package, process, deadline, cleanup, QML, and benchmark evidence passes on Windows, Linux, macOS Intel, and macOS ARM. Milestone 5 added byte-exact typed persistence while capability-rejecting runtime instantiation. `EGUI_DRY_WET_AND_CARLA_TRACKS_PLAN.md` now owns runnable egui dry/wet/Carla delivery and reuses the existing native driver and worker semantics rather than introducing a second protocol. Its cross-target processor catalog deliberately remains empty in the current browser while preserving UI/application mechanics for future browser-native FX. In the rows below, `Partial` describes replacement parity, not the completed shared native baseline.

| ID | Capability or behavior | Current native baseline | Discovery | Current implementation | Evidence |
|---|---|---|---|---|---|
| FX-SUBPROC-001 | Global direct/subprocess policy | QML settings were previously window-owned and Carla was always direct. | Explored for dry/wet/Carla | In progress | Shared native baseline is complete; native egui registration/startup orchestration is Stage 5 of the active dry/wet/Carla plan |
...
| FX-SUBPROC-006 | Pure-egui FX integration | FX chains and dry/wet tracks were deferred. | Explored for dry/wet/Carla | In progress | Native Carla composition, lifecycle/state/log controls, checkpoint fallback, egui executable worker entry, and capability-driven FX status/recovery/log widgets are implemented; recorded-take restore, persistence, and settings remain |
...
| Dry/wet topology and FX-chain hosting/state management | Explored for dry/wet/Carla | In progress under `EGUI_DRY_WET_AND_CARLA_TRACKS_PLAN.md` |
