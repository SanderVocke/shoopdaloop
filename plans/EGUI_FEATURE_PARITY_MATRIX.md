...
| PIANO-XTARGET-001 | Driver-independent track input staging | Native dummy/JACK/CPAL and browser Web Audio/offline ingest bounded side-injected MIDI at frame zero of the next available engine process iteration, without physical MIDI/Web MIDI or a hard realtime latency guarantee | Explored for MIDI piano | MIDI piano required | Planned | Native driver adapters, engine/worklet protocol, no-host browser, bounded-process, and package-isolation evidence |
