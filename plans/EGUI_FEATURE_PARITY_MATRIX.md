...
| LUA-API-001 | Loop selector/query surface | Coordinates are `{track,row}`, sync is `{-1,0}`, and getters preserve list shapes/order | Explored for Lua milestone | Required | Partial | Framework-independent coordinate/list parsing, authoritative snapshots, every legacy function name, all retained key constants, and representative read-your-writes tests pass; exhaustive ported cases remain open |
| LUA-API-002 | Loop mutation/transition surface | Trigger, explicit transition, grab/adopt, gain/balance, selection/target, clear, and repeat-sync | Explored for Lua milestone | Required | Partial | Ordered operations apply through application/backend paths; aligned transition, adopt, repeat-sync, gain/balance, selection/target and clear paths exist; composition and exhaustive parity remain open |
| LUA-API-003 | Track query/mutation surface | Integer/list selectors include sync `-1`; linear gain, fader, balance, mute, and input controls | Explored for Lua milestone | Required | Partial | Complete bindings and conversion/shadow state feed existing track action/backend paths; exhaustive compatibility cases remain open |
| LUA-API-004 | Global control surface | Fixed cycles, solo, sync, play-after-record, and default record/grab are synchronous queries/mutations | Explored for Lua milestone | Required | Partial | Complete bindings provide synchronous read-your-writes and apply through existing global reducers; full retained case table remains open |
...
| LUA-KEY-001 | Keyboard press/release bridge | Qt key/modifier constants, no repeat, and release-sensitive sampler behavior | Explored for Lua milestone | Required | Partial | All retained constants, raw egui-to-Qt translation, typed intents, callback delivery, modifier/repeat/text-entry/focus-loss tests pass; production command workflow remains open |
