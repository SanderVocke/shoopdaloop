...
| CLICK-CAT-001 | Built-in click catalog and defaults | Sorted installed WAV stems; repository defaults resolve to `click_high` primary and `click_low` secondary | Explored for click tracks | Click-track required | Partial | Four embedded WAVs decode in native/Wasm target-neutral tests; application snapshot publishes the stable sorted catalog; dialog remains Stage 2 |
...
| CLICK-AUD-001 | Generated audio loop media | First source channel is resampled, mixed/truncated, copied to every target audio channel, and adopts loop length | Explored for click tracks | Click-track required | Partial | Deterministic 44.1/48 kHz generator and application fake-backend transaction prove all-channel copy, offset/preplay reset, opposite MIDI preservation, stable IDs, and sync/main targets; production playback remains |
| CLICK-MIDI-001 | Generated MIDI loop media | Note-on/off clicks are copied to every target MIDI channel and adopt loop length | Explored for click tracks | Click-track required | Partial | Exact note/order/boundary generator tests and application all-target mutation path prove velocity 127, offset/preplay reset, opposite audio preservation, and stable IDs; production playback remains |
...
| CLICK-PREV-001 | Non-mutating audio preview | Audio draft can be heard without loading the target loop | Explored for click tracks | Click-track required | Partial | Capacity-one PCM outbox, request generations, no-loop-mutation and stale-completion tests pass; native/browser playback adapters remain Stage 3 |
| CLICK-TXN-001 | Transactional target-loop replacement | Replacement must preserve stable IDs, unrelated media/session state, and fail without partial mutation | Explored for click tracks | Click-track required | Partial | Serialized app I/O uses capture/prepare/replace/remap; mixed-content, all-channel, sync/main, conflict/recording, injected replace failure/no-mutation, and stable-ID tests pass; production backend evidence remains |
