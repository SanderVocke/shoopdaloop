...
| XPORT-OWNER-001 | Track and Lua-control app-port ownership | Sync/main tracks and active Lua MIDI registrations publish distinct stable owners; stop/restart churn and owner policy pass | Explored for cross-target ports/Lua | Required | In progress | Native application/GUI evidence passes; real browser global-view workflow remains |
| XPORT-EMPTY-001 | App ports remain visible with no host endpoints | Normalized UI preserves track and Lua-control app ports with empty host inventory; Wasm null MIDI publishes no endpoints | Explored for cross-target ports/Lua | Required | In progress | Backend/application/global-vs-track GUI tests and Wasm compile pass; real browser evidence remains |
...
| XPORT-CTRL-001 | Lua-created logical MIDI ports in global connections | Deterministic script/registration ports, raw-ID MIDI hosts, owner-managed cells, confirmed links, and stop/restart cleanup are implemented | Explored for cross-target ports/Lua | Required | In progress | Native connected/zero-host tests and Wasm compile pass; browser dialog workflow remains |
...
| XLUA-SET-001 | Browser bundled-script settings | Browser registers keyboard-on/APC-off toggles, shows runtime diagnostics, omits user paths/Add-file, and reconciles only active saved revisions | Explored for cross-target ports/Lua | Required | In progress | Registry/settings tests and Wasm compile pass; real `localStorage` save/reload/failure workflow remains |
...
| XPORT-E2E-001 | Cross-target artifacts and regression closure | Production source/Wasm check contains omniLua, scripting, normalized routing, bundled settings, and empty MIDI service; post-change artifacts are unbuilt locally | Explored for cross-target ports/Lua | Required | Planned | Chrome/Firefox/direct-file workflows, realtime gates, workspace/QML suites, dependency/package audits |
