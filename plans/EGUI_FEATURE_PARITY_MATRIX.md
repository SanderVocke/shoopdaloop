...
| LUA-SET-001 | Machine-wide script settings | `script_settings.1` lists bundled/user scripts; keyboard defaults enabled | Explored for Lua milestone | Required | Complete | Typed defaults/load/save and preservation pass; actor startup returns one optional stable ID per configured source, and rejected-slot/duplicate-name tests prove exact path association |
...
| LUA-PRES-001 | Native egui script manager | Add, enable, stop, restart, forget, help, status, errors, listening, and MIDI diagnostics | Explored for Lua milestone | Required | Complete | Plain stable-ID state/actions plus lifecycle/help/log/activity, aggregate and per-rule MIDI endpoint/failure diagnostics, file persistence mapping, browser state, and GUI tests pass |
...
| SET-PRES-001 | Main-menu settings dialog | Registry-generated, resizable dialog with Save/Cancel/reset/help/effect state | Explored for settings | Settings required | Complete | Main-menu routing, stable draft/save/reset tests, diagnostics, and 360×200/900×600 paint |
