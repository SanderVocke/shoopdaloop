...
| TRACK-007 | Dry/wet Add Track choices | QML supports external and Carla processing with dry/wet audio/MIDI topology. | Explored for dry/wet/Carla | Dry/wet/Carla required | In progress | Stages 0–2 of `EGUI_DRY_WET_AND_CARLA_TRACKS_PLAN.md`: typed processor contracts, native External, native Carla composition, role-bearing loops/ports, routing, capture/replacement, and application acceptance are complete; capability-driven egui remains |
...
| FX-SUBPROC-006 | Pure-egui FX integration | FX chains and dry/wet tracks were deferred. | Explored for dry/wet/Carla | In progress | Native Carla composition, lifecycle/state/log controls, checkpoint fallback, and the egui executable worker entry are implemented; capability-driven widgets, persistence, and settings remain |
