...
- The first PR matrix run exposed two packaging-only dependency issues before application compilation: the native-driver feature unnecessarily inherited LV2/Lilv from the full legacy application-backend feature, and Linux/web host jobs lacked JACK development metadata. Native audio now has a narrower engine feature (`jack`/`cpal`/`midir`, no LV2), Linux host jobs install JACK metadata, local warning-denying native/Wasm checks and dependency scans pass, and the correction is awaiting the replacement matrix result.
