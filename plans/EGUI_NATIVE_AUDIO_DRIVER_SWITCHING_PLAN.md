...
- The first PR matrix run exposed two packaging-only dependency issues before application compilation: the native-driver feature unnecessarily inherited LV2/Lilv from the full legacy application-backend feature, and Linux/web host jobs lacked JACK development metadata. Native audio now has a narrower engine feature (`jack`/`cpal`/`midir`, no LV2), and Linux host jobs install JACK metadata. The second run built and packaged every completed platform but exposed an optional real-CPAL engine smoke running without its opt-in on device-less hosts plus a short macOS persistence-test deadline; the smoke is now explicitly opt-in. The third run then reached existing Midir integration tests newly activated by the product feature on a Linux host without `/dev/snd/seq`, so native product jobs now use the repository's explicit environment-aware backend-skip contract. The slow filesystem test allows 30 seconds and reports persistence/switch/pending state on timeout. Local warning-denying debug/release native and Wasm checks, focused tests, and dependency scans pass. The replacement matrix for `4c4f7125` passed all 17 applicable checks; its four platform-condition jobs were expected skips.
