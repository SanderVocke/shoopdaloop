...
The master-settings integration replaces the audit's original temporary QML-format persistence evidence with the fresh typed egui settings registry, one tabbed Settings dialog, and committed-revision runtime reconciliation. Lua API, session, MIDI, and retained QML evidence remains applicable.
...
| 7 | Lifecycle/settings integration; bundled keyboard first-run; user add/reload/remove; atomic preservation; missing/malformed diagnostics | typed `shoop-egui-settings` bundled toggles and ordered path/enabled list; one Settings **Scripts** tab; revision-driven runtime reconciliation; stable startup-ID handshake | 19 settings, 37 GUI, and 12 product tests; committed add/toggle/remove and failed-save no-runtime-change; invalid-before-valid/duplicate-name and rejected-slot association; lifecycle/reload/cleanup suites | Covered |
