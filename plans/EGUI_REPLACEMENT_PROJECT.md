...
| Feature-parity discovery | Partially explored | Tracks/loops, cross-target engine/browser audio, native runtime driver/device settings, connections, session/media persistence, application settings, native External and Carla dry/wet backend topology, the egui executable Carla worker entry, and native Lua/script-created MIDI control are explored and built. Capability-driven dry/wet/FX widgets, Carla settings orchestration, generic MIDI rule editing, and advanced editing remain deferred or largely unexplored. |
...
7. Complete capability-driven dry/wet/FX widgets, persistence, Carla settings, and advanced loop details/editing (native External/Carla backend topology and worker entry are complete).
