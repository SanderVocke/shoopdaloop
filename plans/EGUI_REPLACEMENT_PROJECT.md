...
| Lua scripting and MIDI-controller milestone | In progress | The compatibility contract and detailed matrix are frozen; `shoop_scripting` owns isolated Lua lifecycle and the shared environment used by the retained frontend adapter; native actor integration and non-browser dependency probes pass. Application control bindings, events, MIDI services, settings, presentation, and bundled-script end-to-end work remain open in `EGUI_LUA_SCRIPTING_AND_MIDI_CONTROL_PLAN.md`. |
