...
| Feature-parity discovery | Partially explored | Tracks/loops, cross-target engine/browser audio, native runtime driver/device settings, connections, session/media persistence, application settings, click-track generated media, and the active egui MIDI-piano input slice are explored; native External/Carla dry/wet topology, capability-driven FX UI, role-aware media/state persistence, recorded-take restore, hosting settings, worker entry, and native Lua/script-created MIDI control are built. Generic MIDI rule editing and advanced editing remain deferred or largely unexplored. |
...
| Native dry/wet and Carla tracks | Complete | `EGUI_DRY_WET_AND_CARLA_TRACKS_PLAN.md` delivers capability-driven Regular/Dry + Wet UI, native External/Carla composition, shared routing, public role-bearing ports, exact session/media/current/take state, recovery/log controls, global hosting mode, browser rejection of native-only processors, real JACK/installed-Carla software workflows, 236/236 QML, and a green eight-cell platform matrix. |
| Cross-target Tiny Synth/FX | Complete | The stable `tiny_synth_fx` processor uses pinned dependency-free `tinyviolin 0.2.0` for callback-owned matched-channel audio plus required MIDI on native and AudioWorklet targets. Zero-audio operation, twelve runtime-discovered melodic/percussion presets, pitch bend and modulation wheel, reverb/distortion/compressor/three-band EQ controls, the embedded stable-ID egui editor, smoothed gain, strict portable current/take state, transactional replacement, realtime guards, browser workflows, and native/web packages pass together with the integrated MIDI-keyboard paths. EgUI CI run `31436349108` passes all eight Linux/Windows/macOS/WebAssembly debug/release cells; workspace/QML run `31436349093` passes 1,294 Rust, 236 retained QML, and six packaged Carla subprocess cases. |
| Persistent application settings | Complete | The fresh versioned egui document, typed explicit registration including ordered string/toggle lists and validated string choices, native atomic config storage, browser `localStorage`, and one category-tabbed Settings dialog persist Add Track defaults, native Lua startup, driver profiles, and restart-required global Carla hosting mode without importing QML settings or leaking machine configuration into sessions. |
...
| egui MIDI piano | Complete | `EGUI_MIDI_PIANO_PLAN.md` delivers the details/piano bottom-pane selector, full-range labeled/centered keyboard, paired pointer lifecycle, monitored role-bearing fanout, original-recipient cleanup, and driver-independent protocol-v5 input staging with accepted process-iteration timing. The 181 focused tests, 39 native-driver tests, 23 no-default-feature product tests, warning-denying debug/release native and Wasm/worklet builds, Trunk/self-contained/packages, exact two-track/native recording, scope/dependency audits, and documentation pass. Physical hardware/manual audio and Chrome/Firefox runtime are explicit environment skips, not claimed evidence. |
| egui presentation | Usable | `shoop_egui` renders plain snapshots and emits typed intents while remaining independently browser-compatible and backend/filesystem-free; the capability-driven Dry + Wet form accepts target catalogs and enforces processor shape/MIDI policy, processed headers render only advertised lifecycle/UI/recovery/log facets, Tiny Synth/FX uses an embedded stable-ID editor, loop popups retain stable-ID state and restore compatible take state, and connection/settings surfaces retain typed roles and global hosting configuration. |
...
| Rust business-logic application core | Usable | `shoop_app` runs direct and role-bearing processed tracks/loops, connections, persistence, role-aware media mapping, exact current/recorded FX state, transactional model publication, script/session lifecycle, committed callbacks, and shared GUI/Lua/MIDI reducers through a native actor or bounded cooperative browser runtime. Native session compression is worker-owned; browser audio continues independently in the worklet. |
| Backend façade | Usable | Fake, direct-core dummy/Web Audio, native threaded JACK/CPAL+midir/dummy, and browser-worklet backends expose normalized application/host inventories and confirmed links plus coherent role-bearing audio/MIDI capture, exact-route transactional replacement/migration, cross-target Tiny Synth/FX, native External/Carla composition, state/lifecycle/log controls, bounded generation-tagged transfer, and stable ID remapping. Native switches retain rollback data and browser Chrome/Firefox evidence confirms mutable authoritative worklet routes. |
| Unified egui executable | Usable | `shoopdaloop_egui` dispatches hidden Carla worker mode before GUI startup, applies persisted global Carla hosting and native JACK/CPAL+midir/dummy configuration before backend construction with diagnostic fallback, or starts hosted Web Audio, and supplies target file/settings adapters. Runtime native switches are confirmation-gated, transactionally preserve/resample session contents, and publish full success only after durable preference save. Both targets retain bundled keyboard/APC behavior; browser omits native audio settings, user paths/Add-file, and native MIDI. |
| Cross-target egui CI | Usable | One eight-cell workflow builds, packages, uploads, then tests Linux x86_64, Windows x86_64, macOS arm64, and production WebAssembly in debug/release. PR #683 passes every cell with native-FX prerequisites and hosted/self-contained browser capability workflows. Coverage remains intentionally absent. |
| Browser egui application | Usable | Hosted/direct-file artifacts run negotiated audio host ports, authoritative mutable AudioWorklet routes with built-in Tiny Synth/FX processing, omniLua scripts/settings, and explicit Web MIDI. Supported secure browsers publish physical MIDI endpoints for user-managed direct-track recording/playback and owner-managed Lua control through one bounded hub; unsupported/denied states remain usable. Chrome hosted/direct-file Web MIDI and Firefox audio/no-permission regressions pass; direct-file physical APIs remain browser-policy-dependent. |
...
The historical connections milestone added application/backend descriptors, desired-state actor commands, bounded pending/error handling, live endpoint churn, menu scopes, and a reusable egui matrix. The completed cross-target follow-up replaced per-port candidates with normalized application ports, one host inventory, confirmed/pending links, typed `HostPortId` mutations, and explicit owner metadata. The fixture-only `shoop_egui_preview` was subsequently removed after its presentation coverage moved into `shoop_egui` tests. Native integration now adapts the existing threaded application-backend JACK, CPAL+midir, and dummy ports into the normalized inventory and supports transactional runtime replacement; hosted Web Audio retains production-proven authoritative routing.
...
The persistent-settings slice establishes a fresh `shoop-egui-settings` JSON contract rather than importing QML `settings.1`. Native composition now also registers validated `carla.hosting_mode` with in-process default and restart-required effect; it is applied before backend construction, is not reconciled into running chains, and never enters sessions. `shoop_settings` separates its target-neutral typed registry/codec from opt-in native store and legacy features and supports deterministic ordered string/toggle collections. Consumers explicitly register definitions during composition; `shoopdaloop_egui` loads them before runtime/widget construction, owns storage, and publishes immutable revisions only after save. `shoop_egui` renders categories as tabs in one Settings window. Add Track preferences and bundled keyboard/APC toggles are cross-target; native additionally registers user paths, while browser shows the Scripts tab without file-path actions. Driver/device, Carla/FX, generic MIDI-rule, and session-local settings remain with their owning milestones.

The loop-control refinement keeps temporary controls in presentation state while moving their policies through typed intents and the application owner. Foreground overlays reproduce the QML source/child hover lifetime without changing track geometry; engine and worklet contracts carry balance and atomic grab requests, with ten-second bounded always-on input capture. Fresh production state remains one sync track/loop, and the empty main pane now points to Add Track. The completed dry/wet milestone adds typed processor catalogs, native External dry input/send and wet return/output, dry MIDI, native Carla composition, shared routing, role-aware media/session/driver replacement, current and recorded-take state, capability-driven UI/recovery/logs, hosting settings, and browser-empty transactional rejection. Its closure includes real software JACK and installed-Carla direct/subprocess workflows, all 1,228 workspace tests, 236/236 retained QML cases, and the green eight-cell platform matrix.

The completed native Carla subprocess work establishes reusable frontend-independent semantics for global direct/subprocess policy, one supervised generation per chain, fixed shared-memory audio/MIDI transfer, bounded wet fallback, parent-owned checkpoints, click recovery, generation logs, crash notification, and lifecycle status. Native release evidence covers Windows, Linux, macOS Intel, and macOS ARM. The egui FX milestone now composes these processor/control/realtime boundaries through `shoop_backend`/`shoop_app`, publishes lifecycle/state/log snapshots, dispatches the existing worker protocol from the egui executable before GUI startup, and registers/applies global hosting mode before backend construction. Hosting mode stays out of sessions and supervision stays out of presentation code.

The Tiny Synth/FX follow-up is complete. The integrated implementation passes hosted and self-contained Chrome Web Audio/Web MIDI, hosted Firefox Web Audio, output-only/offline/lifecycle/saturation/stress workflows, debug/release hosted and standalone packaging, warning-denying native/Wasm/worklet builds, realtime/worklet guards, native-FX coverage, sample-rate/native-browser/fresh-runtime state transfer, and zero-audio MIDI routing. EgUI CI run `31436349108` passes all eight Linux/Windows/macOS/WebAssembly debug/release cells. Workspace/QML run `31436349093` passes 1,294/1,294 Rust tests, 236/236 retained QML cases, and 6/6 packaged Carla subprocess cases. The local host still lacks Qt and physical audio/MIDI hardware; those local/manual surfaces are not claimed, because authoritative Qt-enabled CI and deterministic software/browser tests provide the required gates.

The abandoned path that rendered egui inside the QML application has been deleted. The legacy product continues with its ordinary QML loop presentation and Rust/CXX-Qt frontend, while `shoopdaloop_egui` remains standalone and has no QML/frontend dependency. The fixture-only `shoop_egui_preview` has been removed. Formatting, warning-denying all-target builds, focused suites, standalone Wasm checks, the current 1,228-test workspace archive, and 236-case QML matrix pass on the recorded native release surfaces. Release-browser and cross-platform gates are accepted as passing under the user's explicit documentation-closure instruction. This cleanup does not advance the whole-application parity roadmap or the final Qt deletion gate.
...
7. Role-aware dry/wet/FX persistence, Carla settings, and processed loop details/actions. Complete under `EGUI_DRY_WET_AND_CARLA_TRACKS_PLAN.md`; unrelated advanced/composite editing remains in item 8.
...
9b. Explicit Web MIDI permission, direct-track MIDI recording/playback, and browser Lua controller I/O. Complete under `EGUI_WASM_WEBMIDI_PLAN.md`.
