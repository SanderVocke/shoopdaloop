...
| Unified egui executable | Usable | `shoopdaloop_egui` selects native dummy or hosted Web Audio, supplies target file/settings adapters, loads settings before consumers, and publishes new values only after durable save. Both targets register/reconcile bundled keyboard/APC settings; browser omits native user paths/Add-file and runs against empty MIDI. Native workflows and historical browser workflows exercise settings/session/media; fresh combined browser automation remains open. |
...
| Cross-target ports, browser Lua, and omniLua migration | In progress | Pinned omniLua is workspace-wide and native/frontend/QML evidence passes. Normalized protocol-v3 Web Audio routes, deterministic owner-managed Lua MIDI ports, browser bundled settings, cooperative scripting, key dispatch, source-bearing sessions, and empty MIDI all compile in production Wasm with native unit/integration evidence. Real browser execution, artifacts/package scans, and final regressions remain open. |
