...
| Lua scripting and MIDI-controller milestone | In progress | The runtime, actor integration, broad compatibility surface, keyboard/APC workflows, native/fake MIDI, settings/session support, and egui manager are implemented. The evidence-based audit in `EGUI_LUA_SCRIPTING_AND_MIDI_CONTROL_COMPLETION_AUDIT.md` reopened exhaustive retained-API coverage, strict output pacing and granular MIDI diagnostics, serial composition timing, callback/timer edge cases, and invalid-startup path/ID mapping. Generic MIDI-rule editing remains separate. |
...
9. Native Lua scripting, keyboard control, and script-created MIDI control. In progress under `EGUI_LUA_SCRIPTING_AND_MIDI_CONTROL_PLAN.md` and its completion audit; generic MIDI rule editing remains separate.
