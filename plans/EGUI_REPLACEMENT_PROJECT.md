...
| Fifth major milestone | In progress | Implementation and native/browser persistence evidence are complete; the final retained-QML regression result is pending authoritative PR CI because the local Qt harness reproduces the same load failure on clean `origin/master`. |
