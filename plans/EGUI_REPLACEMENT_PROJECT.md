...
| Click-track generated media | In progress | Stages 0–3 of `EGUI_CLICK_TRACK_GENERATION_PLAN.md` deliver embedded checked audio/MIDI generation, the stable-ID egui dialog, transactional native/worklet replacement, bounded native/browser preview adapters, exact export/save-load evidence, and debug native/Chrome/Firefox/self-contained workflows. Release/full-regression closure remains. |
...
6a. Generated audio/MIDI click-loop content, loop-length fitting, and non-mutating native/browser preview. Planned under `EGUI_CLICK_TRACK_GENERATION_PLAN.md`.
