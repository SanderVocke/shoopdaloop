...
| Feature-parity discovery | Partially explored | Tracks/loops, cross-target engine/browser audio, native runtime driver/device settings, connections, session/media persistence, application settings, native External and Carla dry/wet backend topology, capability-driven dry/wet/FX widgets, the egui executable Carla worker entry, and native Lua/script-created MIDI control are explored and built. Role-aware processed-track persistence, Carla settings orchestration, generic MIDI rule editing, and advanced editing remain deferred or largely unexplored. |
...
The loop-control refinement keeps temporary controls in presentation state while moving their policies through typed intents and the application owner. Foreground overlays reproduce the QML source/child hover lifetime without changing track geometry; engine and worklet contracts carry balance and atomic grab requests, with ten-second bounded always-on input capture. Fresh production state remains one sync track/loop, and the empty main pane now points to Add Track. The active dry/wet milestone has since delivered typed processor contracts and native External dry input/send, wet return/output, dry MIDI, routing, media capture, and transactional replacement; capability-driven egui and Carla composition remain in progress.

The completed native Carla subprocess work establishes reusable frontend-independent semantics for global direct/subprocess policy, one supervised generation per chain, fixed shared-memory audio/MIDI transfer, bounded wet fallback, parent-owned checkpoints, click recovery, generation logs, crash notification, and lifecycle status. Native release evidence covers Windows, Linux, macOS Intel, and macOS ARM. The active egui FX milestone now composes these processor/control/realtime boundaries through `shoop_backend`/`shoop_app`, publishes lifecycle/state/log snapshots, and dispatches the existing worker protocol from the egui executable before GUI startup. Carla-specific settings registration and the capability-driven widgets remain; hosting mode stays out of sessions and supervision stays out of presentation code.
