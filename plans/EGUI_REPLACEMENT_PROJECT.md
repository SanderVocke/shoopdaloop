...
| Feature-parity discovery | Partially explored | Tracks/loops, cross-target engine/browser audio, connections, and the completed native Carla subprocess baseline are built. Session persistence and individual loop audio/MIDI I/O are explored and planned in Milestone 5 around fresh `.shoop` v1; egui FX/settings composition, scripting, MIDI control, and advanced editing remain deferred or largely unexplored. |
...
| Fifth major milestone | Planned | `EGUI_MILESTONE_5_SESSION_PERSISTENCE_AND_LOOP_IO.md` requires playback-safe cross-target session save/load, exact arbitrary-channel media, loop audio/MIDI I/O, resampling, and native/browser file services. The format starts fresh and intentionally does not import QML-era archives. |
...
| Rust business-logic application core | Usable | `shoop_app` runs the same tracks/loops and authoritative track-port connection model through a native actor or bounded cooperative browser runtime, including selection/target/sync/solo-aware grab and dry-mode policy. |
| Backend façade | Usable | Native dummy and browser-worklet operations include coherent loop gain/balance and bounded audio/MIDI always-on capture adoption. Native dummy also provides typed external discovery/mutation; hosted Web Audio explicitly reports arbitrary external connection management unavailable while retaining default physical routing. |
...
| Cross-target egui CI | In progress | One matrix workflow builds, packages, uploads, then tests Linux x86_64, Windows x86_64, macOS arm64, and production WebAssembly in debug/release. Local Linux/web `act` paths and browser workflows pass; GitHub-hosted cross-platform/cache/upload evidence is pending. Coverage is intentionally absent. |
| Browser egui application | Usable | Hosted secure artifacts request default microphone audio after an explicit gesture and render through AudioWorklet; the self-contained file exposes only explicit offline dummy operation. The production bundle includes the complete Web Audio path and connections dialog and is not a feature-preview build. |
...
Milestone 5 is planned around a real `shoop_session` boundary and the fresh `.shoop` v1 contract in `docs/session_format_v1.md`. The contract uses exact per-channel floating-point payloads, exact integer-frame MIDI, explicit versions/hashes, transactional load, and cross-target file adapters. QML-era `.shl`, `session.1`, tar archives, and JSON `.smf` are intentionally unsupported rather than migration inputs; QML remains only a behavior-discovery and regression source.
...
5. Session persistence and media workflows required to use the tracks/loops slice across runs. Planned by `EGUI_MILESTONE_5_SESSION_PERSISTENCE_AND_LOOP_IO.md` using the fresh `.shoop` v1 format.
