...
| Cross-target ports, browser Lua, and omniLua migration | In progress | The workspace, shared scripting crate, and retained frontend/QML bridge now compile against pinned omniLua 0.7.1; native scripting and focused frontend tests pass, the shared crate compiles for Wasm without native MIDI, and former C Lua dependencies are absent from manifests/lockfile/trees. Browser execution and retained QML end-to-end gates remain open before port normalization and mutable Web Audio routing can claim completion. |
