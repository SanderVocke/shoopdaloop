...
| Backend façade | Usable | Fake, native dummy, and browser-worklet backends expose normalized application/host inventories and confirmed links plus coherent arbitrary-channel audio/MIDI session capture, exact-route transactional replacement/migration, bounded generation-tagged transfer, and stable ID remapping. Browser worklet routes are mutable and authoritative in source; real browser regression evidence remains open. |
| Unified egui executable | Usable | `shoopdaloop_egui` selects native dummy or hosted Web Audio, supplies target file/settings adapters, loads settings before consumers, and publishes new values only after durable save. Native Lua startup/runtime reconciliation follows committed settings revisions; browser composition now starts embedded keyboard/APC definitions on the cooperative owner with an empty MIDI service, while browser script-setting registration remains open. Native and browser workflows exercise settings and produced session/audio/MIDI bytes. |
...
| Browser egui application | Usable | Hosted secure artifacts request microphone or output-only audio after an explicit gesture and render through AudioWorklet. Production Wasm now configures negotiated capture/destination host ports and bounded mutable routes before replaying track/link commands. Hosted and self-contained artifacts provide async session/media upload/download and origin-scoped settings; direct-file physical audio/storage remain browser-policy-dependent, and fresh post-change automation remains open. |
| Cross-target ports, browser Lua, and omniLua migration | In progress | Pinned omniLua is workspace-wide and native/frontend/QML evidence passes. Normalized ports and protocol-v3 authoritative Web Audio routes are implemented with allocation-free signal-flow, empty-host, failure, exact-session, and migration tests; production Wasm compiles them alongside cooperative scripting, embedded keyboard/APC, key dispatch, source-bearing sessions, and empty MIDI. Lua control-port publication, browser script settings, real browser execution, and package evidence remain open. |
...
The historical connections milestone added application/backend descriptors, desired-state actor commands, bounded pending/error handling, live endpoint churn, menu scopes, and a reusable egui matrix. The active cross-target follow-up has now replaced per-port candidates with normalized application ports, one host inventory, confirmed/pending links, typed `HostPortId` mutations, and explicit owner metadata. `shoop_egui_preview` remains fixture-only. Native integration uses the engine dummy registry; hosted Web Audio now has source-level authoritative routing, while native real-driver patching remains deferred.

The active cross-target ports, browser Lua, and omniLua milestone supersedes those two browser limitations without adding Web MIDI or native egui real-driver selection. Normalized ownership/inventory and mutable worklet audio routes are implemented; Lua-created logical MIDI publication, browser settings, real hosted/standalone workflows, and final package scans remain. One pinned omniLua Lua 5.4 runtime serves `shoop_scripting` and retained frontend/QML, and former C Lua dependencies are removed. The remaining contract and blocked stop condition are in `EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md`.
...
6. Persistent application-settings foundation, category-tabbed dialog, Add Track preferences, and native Lua startup settings. Complete. Remaining track topology, native real-driver/device management, and other feature-specific settings stay in later milestones.
...
9a. Project-wide `mlua`-to-omniLua migration, cross-target application/host port normalization, mutable Web Audio routes, visible empty-host MIDI/control ports, and browser Lua/bundled keyboard support. Planned under `EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md`; Web MIDI remains separate.
