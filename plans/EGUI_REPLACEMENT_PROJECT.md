...
| Feature-parity discovery | Partially explored | Tracks/loops, cross-target engine/browser audio, native runtime driver/device settings, connections, session/media persistence, application settings, click-track generated media, native External/Carla dry/wet topology, capability-driven FX UI, role-aware media/state persistence, recorded-take restore, hosting settings, worker entry, and native Lua/script-created MIDI control are explored and built. Generic MIDI rule editing and advanced editing remain deferred or largely unexplored. |
