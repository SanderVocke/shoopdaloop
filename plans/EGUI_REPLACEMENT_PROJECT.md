...
| Native dry/wet and Carla tracks | Complete | `EGUI_DRY_WET_AND_CARLA_TRACKS_PLAN.md` delivers capability-driven Regular/Dry + Wet UI, native External/Carla composition, shared routing, public role-bearing ports, exact session/media/current/take state, recovery/log controls, global hosting mode, browser-empty capability rejection, real JACK/installed-Carla software workflows, 236/236 QML, and a green eight-cell platform matrix. |
...
| egui presentation | Usable | `shoop_egui` renders plain snapshots and emits typed intents while remaining independently browser-compatible and backend/filesystem-free; the capability-driven Dry + Wet form accepts native or synthetic processor catalogs, processed headers render only advertised lifecycle/UI/recovery/log facets, loop popups retain stable-ID state and restore compatible take state, and connection/settings surfaces retain typed roles and global hosting configuration. |
