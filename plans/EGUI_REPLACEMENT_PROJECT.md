...
| Lua scripting and MIDI-controller milestone | Complete | `shoop_scripting` owns isolated lifecycle, the retained API/environment, callbacks, bounded timers, and fake/native MIDI services. Native application reducers, egui key routing/manager, preservation-aware settings, session scripts, embedded resources, production keyboard/APC workflows, hotplug/reconnect, diagnostics, browser rejection, and dependency/realtime gates pass under `EGUI_LUA_SCRIPTING_AND_MIDI_CONTROL_PLAN.md`. Generic MIDI-rule editing remains separate. |
...
9. Native Lua scripting, keyboard control, and script-created MIDI control. Complete under `EGUI_LUA_SCRIPTING_AND_MIDI_CONTROL_PLAN.md`; generic MIDI rule editing remains separate.
