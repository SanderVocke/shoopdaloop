...
| egui MIDI piano | Usable | `EGUI_MIDI_PIANO_PLAN.md` delivers the details/piano bottom-pane selector, full-range labeled/centered keyboard, paired pointer lifecycle, monitored role-bearing fanout, original-recipient cleanup, and driver-independent protocol-v5 input staging. Exact two-track engine and native-product recording, 78 presentation tests, 23 no-default-feature product tests, Wasm/worklet, Trunk, self-contained/package, and dependency-isolation checks pass; browser runtime execution is an explicit no-browser environment skip pending final closure. |
