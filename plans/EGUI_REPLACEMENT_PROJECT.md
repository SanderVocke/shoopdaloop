...
| Framework-independent application API | Complete | `shoop_app_api` owns stable IDs, immutable connection/task/warning/mapping/script/MIDI-diagnostic views, desired-state, key, script, and file-workflow intents, capabilities, and notifications without framework/backend dependencies. |
...
| Cross-target ports, browser Lua, and omniLua migration | In progress | The workspace, shared scripting crate, and retained frontend/QML bridge use pinned omniLua 0.7.1. Native scripting, frontend Rust tests, and all Lua-specific retained QML self-tests pass. The production browser composition now compiles the cooperative scripting owner, embedded keyboard/APC definitions, key dispatch, source-bearing session transactions, and empty MIDI service; browser execution/package evidence, settings UX, port normalization, and mutable Web Audio routing remain open. |
...
The planned cross-target ports, browser Lua, and omniLua migration milestone intentionally supersedes those two browser limitations without adding Web MIDI or native egui real-driver selection. It normalizes application-port ownership and host endpoint inventory, turns the admitted microphone/destination channels into authoritative mutable worklet routes, includes Lua-created logical MIDI ports in the global connection view, and mandates one pinned omniLua Lua 5.4 runtime across `shoop_scripting` and the retained frontend/QML Lua bridge. Completion requires native, `wasm32-unknown-unknown`, and retained QML compatibility evidence plus removal of `mlua` and its C runtime dependency chain from the workspace. The full contract and blocked stop condition are in `EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md`.
