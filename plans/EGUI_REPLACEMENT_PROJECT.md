...
| Native dry/wet and Carla tracks | Final validation | Capability-driven Regular/Dry + Wet UI, native External/Carla composition, shared routing, public role-bearing ports, exact session/media/current/take state, recovery/log controls, global hosting mode, and browser-empty capability rejection are implemented under `EGUI_DRY_WET_AND_CARLA_TRACKS_PLAN.md`; final workspace/QML/browser/package/manual evidence remains in progress. |
...
| Browser egui application | Usable | Hosted/direct-file release artifacts run negotiated capture/destination host ports, authoritative mutable AudioWorklet routes, omniLua keyboard/session scripts, APC zero-host control ports, and persisted bundled settings. Chrome hosted/direct-file and Firefox hosted workflows pass; direct-file physical audio/storage remain browser-policy-dependent. |
...
The loop-control refinement keeps temporary controls in presentation state while moving their policies through typed intents and the application owner. Foreground overlays reproduce the QML source/child hover lifetime without changing track geometry; engine and worklet contracts carry balance and atomic grab requests, with ten-second bounded always-on input capture. Fresh production state remains one sync track/loop, and the empty main pane now points to Add Track. The dry/wet milestone has since delivered typed processor catalogs, native External dry input/send and wet return/output, dry MIDI, native Carla composition, shared routing, role-aware media/session/driver replacement, current and recorded-take state, capability-driven UI/recovery/logs, hosting settings, and browser-empty transactional rejection; final validation remains in progress.
...
7. Complete role-aware dry/wet/FX persistence, Carla settings, and advanced loop details/editing (native External/Carla backend topology, capability-driven widgets, and worker entry are complete).
...
9a. Project-wide `mlua`-to-omniLua migration, cross-target application/host port normalization, mutable Web Audio routes, visible empty-host MIDI/control ports, and browser Lua/bundled keyboard support. Complete under `EGUI_WEB_PORTS_AND_WASM_LUA_PLAN.md`; Web MIDI remains separate.
