...
| Cross-target Tiny Synth/FX | In progress | The stable `tiny_synth_fx` processor uses pinned dependency-free `tinyviolin 0.1.0` for callback-owned matched-channel audio plus required MIDI on native and AudioWorklet targets. Zero-audio operation, runtime presets, typed controls, the embedded stable-ID egui editor, smoothed gain, strict portable current/take state, transactional replacement, realtime guards, browser workflows, and native/web packages are implemented and focused-tested. The diverged `origin/master` MIDI-keyboard/`shoop_engine` work is integrated; closure awaits clean serialized broad tests of the combined MIDI paths, retained Qt/QML gates, and the authoritative eight-cell CI matrix. |
...
The Tiny Synth/FX follow-up is implemented but not yet closed. Current evidence includes hosted Chrome 147 Web Audio/Web MIDI, hosted Firefox 153 Web Audio, Chrome self-contained offline/output-only operation, debug/release hosted and standalone packages, warning-free no-default native/Wasm checks, warning-free default native-FX compilation, realtime/worklet guards, and 42/42 native-FX backend tests. After integration, the combined protocol/worklet/backend/engine/app/egui/session no-default-feature suites pass serially, including 610 engine tests and realtime guards, and the no-default-feature product suite passes 23/23; the earlier two timeout-sensitive failures under concurrent Cargo contention did not reproduce. This host lacks Qt, so the all-target workspace build stops at CXX-Qt discovery and retained QML cannot run. The `origin/master` MIDI keyboard and related engine changes are now integrated. Full serialized combined-path tests, retained QML, and the authoritative eight-cell matrix are required before changing the milestone to Complete.
