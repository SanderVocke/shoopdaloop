...
| Feature-parity discovery | Partially explored | Tracks/loops now include the QML hover-control families, grab policy, and stereo loop balance; cross-target dummy-engine, browser physical audio, and track-port connections are also explored and built. Carla subprocess policy, lifecycle, recovery, diagnostics, and realtime transport are explored as a completed shared native baseline, while egui settings/FX composition, broader persistence, scripting, MIDI control, and advanced editing remain deferred or largely unexplored. |
