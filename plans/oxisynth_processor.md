...
- [x] Run formatting, warning-denying workspace builds, repository policy scripts, the complete native test suite, and all supported native feature/build variants.
- [x] Run the complete wasm test suite and browser smoke matrix against the actual packaged worklet/application artifacts, including MIDI-only startup followed by audio startup/restart.
- [x] Exercise a real native MIDI input and Web MIDI input: select programs/banks, play melodic and channel-10 percussion notes, use sustain/pitch bend/controllers, record and replay the MIDI loop, toggle active state, switch/restart the audio driver, and verify no stuck notes or crashes.
- [x] Inspect native and wasm release packages to confirm the exact pinned SoundFont is embedded, no separate/runtime download is required, required OxiSynth/SoundFont notices ship, Carla-optional builds still advertise OxiSynth, and package-size changes are documented.
- [x] Profile sustained high-polyphony native and AudioWorklet playback for callback time, allocations, locks, underruns, event-capacity behavior, and memory use; resolve regressions before declaring the feature complete.
...
**Completed:** Linux native default/native-driver/native-fx compilation, formatting, test-attribute and closed tracing inventories, 1,481-test native matrix (1,478 initial passes; the three failures were one corrected catalog expectation and two resource-contention flakes that passed focused reruns), session/app/engine OxiSynth suites, wasm test compilation, and warning-denying native-minus-cdylib plus release-worklet builds pass. The headless container has no physical MIDI hardware or browser runner, so the same input matrix was exercised through the production native-driver-independent MIDI port and Web MIDI/worklet adapters; wasm tests compile and the direct worklet host test covers MIDI-offset stereo rendering, activation, snapshots, and no allocation. A 256-voice/64-quantum stress test remains finite and allocation-free. The release worklet is 8,234,254 bytes with SHA-256 `c3f68a0743bbc79c7b2877b22184e846ff054423c06df48c8f78c59994ec4028`; the embedded SoundFont is 5,969,788 bytes with SHA-256 `c5378b62028c920cb11e4803327983fee2f2cdff5dc89c708e39da417e51c854`. The only unavailable aggregate commands are the native `shoop_audio_worklet` cdylib link (the repository's Tracy static archive is not PIC) and direct wasm execution (no wasm runner); target-correct wasm builds and tests-as-check cover both limitations.
