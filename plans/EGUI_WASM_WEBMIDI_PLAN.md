...
|---:|---|---|---|
| 1 | `browser_midi.rs`: `BrowserMidiState`, gesture-only `request_access`, asynchronous `open` observation, send-error reporting, and unsupported/denied/retry/SysEx button copy | Hub lifecycle test; Chrome granted, `WEB_MIDI_DENY_FIRST=1`, `WEB_MIDI_OPEN_FAIL=1`, and forced send-failure paths; Firefox truthful unrequested state | Complete |
...
| 3 | `WebAudioBackend` sends route mutations through protocol v4; `EngineBackend` alone mutates worklet route truth and publishes confirmed links separately from desired state | Backend disconnect/reconnect and missing-endpoint tests; worklet normalized-route tests; Chrome waits for authoritative track confirmations after mutation, load, hotplug, and restart | Complete |
...
| 7 | `WebMidiControlService` implements the shared `MidiControlService`; `ScriptManager` remains authoritative for regex, pacing, fanout, reconnect, teardown, and diagnostics | Existing scripting hotplug/multi-match/rate/zero-rate/teardown tests; unchanged APC production Chrome input, authoritative solo action, and LED output before and after hotplug | Complete |
...
| 13 | `browser_smoke.mjs` installs a pre-start `navigator.requestMIDIAccess` platform double while production still executes the direct `web-sys` adapter; no fixture backend exists in product composition | Hosted release Chrome covers discovery, mutation, record/load/playback, APC I/O, denial/retry, open/send failure, overflow, hotplug, restart, and callback continuity; self-contained release repeats the full workflow | Complete |
| 14 | Native paths remain target-gated; Wasm manifests use direct `web-sys`; package verification and dependency scans reject missing MIDI markers and native backend leakage | Warning-denying native/Wasm/worklet builds; 1,203-test workspace; 236/236 QML; Chrome ordinary/output-only/settings/session/lifecycle/stress/direct-file regressions; Firefox regression; debug/release package, no-import, and forbidden-tree scans | Complete |
...
- Focused suites passed before the full `SHOOP_ALLOW_MISSING_BACKENDS=1 cargo test --workspace --features shoop_engine/app_backend --no-fail-fast` run; that full run reported 1,203 passed and no failed summary.
...
- The implementation and browser verification milestone is commit `b1dca267`; documentation and closure is commit `cd202912`. Pre-existing unrelated `src/rust/shoop_egui` working-tree edits were excluded from both commits.
