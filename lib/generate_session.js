function generate_audio_port(id, passthrough_to_ids, name_parts, direction, volume) {
    return {
        'id': id,
...
function generate_midi_port(id, passthrough_to_ids, name_parts, direction) {
    return {
        'id': id,
        'passthrough_to': passthrough_to_ids,
        'schema': 'midiport.1',
...
function generate_default_track(
    ...
    var audio_dry_port_pairs = Array.from(Array(n_audio_dry).keys()).map((idx) => {
        ...
        return [
            generate_audio_port(in_id, [out_id], [port_name_base, in_id_post], 'input', 1.0),
            generate_audio_port(out_id, [], [port_name_base, out_id_post], 'output', 1.0),
        ]
    })
    ...
    var midi_direct_port_pairs = have_midi_direct ? (() => {
        ...
        var out_id = id + out_id_post;
        return [[
            generate_midi_port(in_id, [out_id], [port_name_base, in_id_post], 'input'),
            generate_midi_port(out_id, [], [port_name_base, out_id_post], 'output'),
        ]]
    })() : [];
    ...
    for (var i=0; i<n_loops; i++) {
        ...
        audio_direct_port_pairs.forEach((pair, idx) => {
            channels.push(generate_loop_channel(
                id + '_loop_' + ii + "_audio_direct_" + idx.toString(), 'direct', 'audio', 0, pair.map(p => p.id)
            ))
        })
        midi_direct_port_pairs.forEach((pair, idx) => {
            console.log("PAIR", pair)
            channels.push(generate_loop_channel(
                id + '_loop_' + ii + "_midi_direct_" + idx.toString(), 'direct', 'midi', 0, pair.map(p => p.id)
            ))
        })
        audio_dry_port_pairs.forEach((pair, idx) => {
            channels.push(generate_loop_channel(
                id + '_loop_' + ii + "_audio_dry_" + idx.toString(), 'dry', 'audio', 0, pair.map(p => p.id)
            ))
        })
        midi_dry_port_pairs.forEach((pair, idx) => {
            channels.push(generate_loop_channel(
                id + '_loop_' + ii + "_midi_dry_" + idx.toString(), 'dry', 'midi', 0, pair.map(p => p.id)
            ))
        })
