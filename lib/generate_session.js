...
function generate_default_track(name, n_loops, id, first_loop_is_master) {
    var audio_l_ports = [
        generate_audio_port(id+'_audio_l_in', [id+'_audio_l_out'], [id, '_audio_in_l'], 'input', 1.0),
        generate_audio_port(id+'_audio_l_out', [], [id, '_audio_out_l'], 'output', 1.0),
    ];
    var audio_r_ports = [
        generate_audio_port(id+'_audio_r_in', [id+'_audio_r_out'], [id, '_audio_in_r'], 'input', 1.0),
        generate_audio_port(id+'_audio_r_out', [], [id, '_audio_out_r'], 'output', 1.0),
    ];
    var midi_ports = [
        generate_midi_port(id+'_midi_in', [id+'_midi_out'], [id, '_in_midi'], 'input'),
        generate_midi_port(id+'_midi_out', [], [id, '_out_midi'], 'output'),
    ];
    var loops = []
    for (var i=0; i<n_loops; i++) {
        ...
        var loop = generate_loop(id+'_loop_'+ii, 0, (first_loop_is_master && i==0) ? true : false, [
