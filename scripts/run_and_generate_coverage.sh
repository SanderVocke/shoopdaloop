...
# Profraw reporting
c="find . -type f \\( -name "*.so" -o -executable -exec file {} + | grep "ELF" | cut -d: -f1 \\) -print0 | xargs -0 ${_LLVM_COV} export -instr-profile=coverage.profdata -format=lcov > ${_REPORTDIR}/${_LLVM_REPORTNAME}.info"
echo "---------------------------------------"
