...
- [x] Record the PR URL and validated review-fix head in this plan: https://github.com/SanderVocke/shoopdaloop/pull/844 at `9e2062f9` before this closure-record-only commit.
...
Closure evidence (2026-09-01): PR https://github.com/SanderVocke/shoopdaloop/pull/844. Build and Test run `33500456785` passed Rust coverage and all eight Linux/macOS/Windows/web debug/release jobs. The web-debug job passed the required complete Node and policy-triggered Chromium suites plus hosted/self-contained Chrome AudioWorklet and multirealm Perfetto smokes; web release passed Firefox Web Audio. Docs run `33500456549`, CodeQL run `33500456586`, and both Codecov statuses passed. Codecov's informational 97.07% patch-coverage report, with project coverage increasing 0.14 points, was reviewed and answered at issue comment `5493247764`. After the draft was marked ready, Codex raised three valid findings across two review cycles: unchanged browser protocol versioning, missing canonical-envelope validation for recorded Built-in FX states, and silence when a runtime callback grows beyond the processor's prepared block size. Review-fix commit `63f13a45` bumps the protocol and raw-host contract to version 20 and validates every recorded Built-in FX state before publication. Review-fix commit `9e2062f9` chunks any larger routed callback through preallocated processor storage and adds an enabled/disabled callback-growth regression test. Warning-denying builds, focused native/Node tests, the full 1,658-test native suite, the full 1,377-test Node suite, locked Wasm builds, Trunk, zero-import worklet, and raw-host contract all pass. The final replacement CI and review-thread resolution are required on the closure-record-only child commit before completion.
